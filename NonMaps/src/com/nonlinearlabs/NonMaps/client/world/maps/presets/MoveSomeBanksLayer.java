...
public class MoveSomeBanksLayer extends MapsControl {
	...
	public void draw(Context2d ctx, int invalidationMask) {
		...
		if(currentPosition.getX() > startPosition.getX())
		{
			...
			{
				minRect	= new Rect(startPosition.getX(), startPosition.getY(), 
						currentPosition.getX() - startPosition.getX(), 
						currentPosition.getY() - startPosition.getY());
			}
			else
			{
				minRect = new Rect(startPosition.getX(), currentPosition.getY(), 
						currentPosition.getX() - startPosition.getX(),
		...
		else
		{
			if(currentPosition.getY() > startPosition.getY())
			{
				minRect	= new Rect(currentPosition.getX(), startPosition.getY(), 
						startPosition.getX() - currentPosition.getX(), 
						currentPosition.getY() - startPosition.getY());
			}
			else
			{
				minRect = new Rect(currentPosition.getX(), currentPosition.getY(), 
						startPosition.getX() - currentPosition.getX(),
						startPosition.getY() - currentPosition.getY());
			}
		}
