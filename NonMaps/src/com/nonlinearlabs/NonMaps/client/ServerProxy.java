...
public class ServerProxy {
	...
	private void applyChanges(String responseText) {
		...
		nonMaps.getNonLinearWorld().getViewport().getOverlay().update(settingsNode, editBufferNode, presetManagerNode,
				deviceInfo, undoNode);
	...
	public void onParameterChanged(Parameter pl) {
		...
		StaticURI uri = new StaticURI(path, new StaticURI.KeyValue("id", pl.getParameterID()),
				new StaticURI.KeyValue("value", pl.getValue().getQuantizedClipped()));
	...
	public void getDifferencesOf2PresetsAsCsv(String uuid1, String uuid2, DownloadHandler handler) {
		downloadFile("/presets/get-diff?p1=" + URL.encodeQueryString(uuid1) + "&p2=" + URL.encodeQueryString(uuid2),
				handler);
	}
	...
	private void doImportBank(String fileName, String text, int lastModified, NonPosition p) {
		...
		StaticURI uri = new StaticURI(path, new StaticURI.KeyValue("xml", text), new StaticURI.KeyValue("x", p.getX()),
				...
				new StaticURI.KeyValue("lastModified", lastModified));
		final XMLHttpRequest xhr = XMLHttpRequest.create();
	...
	public void movePresetToEmptyBank(IPreset p, Bank b) {
		...
		StaticURI uri = new StaticURI(path, new StaticURI.KeyValue("preset", p.getUUID()),
				new StaticURI.KeyValue("bank", b.getUUID()));
		queueJob(uri, false);
	}
	...
	public void insertBankInEmptyBank(Bank target, Bank origin) {
		...
		StaticURI uri = new StaticURI(path, new StaticURI.KeyValue("targetbank", target.getUUID()),
				new StaticURI.KeyValue("originbank", origin.getUUID()));
		queueJob(uri, false);
	}
	...
	public void copyPresetInNewBank(IPreset preset, Bank bank) {
		...
		StaticURI uri = new StaticURI(path, new StaticURI.KeyValue("preset", preset.getUUID()),
				new StaticURI.KeyValue("bank", bank.getUUID()));
		queueJob(uri, false);
	}
}
