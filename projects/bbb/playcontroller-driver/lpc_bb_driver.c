...
#define LPC_BB__DRIVER_MAX_SPEED    6000000  // was 3000000
#define LPC_BB__DRIVER_XFER_SIZE    1024
#define LPC_BB__DRIVER_DEV_MAJOR    300
#define LPC_BB__DRIVER_RX_BUFF_SIZE 1024  // must be 2^n, for buffer mask to work !
#define LPC_BB__DRIVER_RX_BUFF_MASK (LPC_BB__DRIVER_RX_BUFF_SIZE - 1)
...
static u8 *     tx_buff;                 // [2][LPC_BB__DRIVER_XFER_SIZE]  current buffer is user write buffer, the other is the SPI xfer buffer
...
static u8 *rxb;                             // [LPC_BB__DRIVER_XFER_SIZE]   SPI RX buffer
...
static u8 *rx_buff;                         // [LPC_BB__DRIVER_RX_BUFF_SIZE]   user RX ring buffer
static u16 rx_buff_head;
static u16 rx_buff_tail;

static u8 no_hb_count;       // number of lpc_bb_ heartbeats currently missed
#define MAX_NO_HB_COUNT (3)  // number of missed lpc_bb_ heartbeats allowed before error / write stall
static u8  busy;
static u8  xfer_repeat;  // flag repeat request (when lpc_bb_ was offline)
...
// **************************************************************************
// check if lpc_bb_ heartbeat pin toggles after 125us max
//
...
// that required by the 125us toggle rate of the lpc_bb_ (which is precise as it
...
static int lpc_bb_driver_check_hb(int hb_gpio)
{
  ...
  return 0;  // no change, lpc_bb_ is busy/dead
}
...
static void lpc_bb_driver_package_parse(void)
{
  ...
  if (!rx_bytes_to_read || (rx_bytes_to_read > LPC_BB__DRIVER_RX_BUFF_SIZE - 4))
    return;  // no data, or message can't fit in buffer

  mutex_lock(&rx_buff_lock);
  if (rx_bytes_to_read < (LPC_BB__DRIVER_RX_BUFF_SIZE - bytes_in_rx_buffer()))
  {  // enough free space in buffer
    ...
    if (rx_buff_head + rx_bytes_to_read > LPC_BB__DRIVER_RX_BUFF_SIZE)
    {  // write beyond physical buffer end, so split in two chunks
      first  = LPC_BB__DRIVER_RX_BUFF_SIZE - rx_buff_head;
...
static u8 *lpc_bb_driver_get_txb(void)
{
  ...
  txb                    = tx_buff + tx_write_buffer * LPC_BB__DRIVER_XFER_SIZE;  //[tx_write_buffer];
  tx_write_buffer        = (tx_write_buffer + 1) % 2;                             // switch buffers
  pSignature             = (u32 *) (tx_buff + tx_write_buffer * LPC_BB__DRIVER_XFER_SIZE);
...
static int lpc_bb_driver_transfer(struct spi_device *dev, u8 *txb)
{
  ...
  xfer.tx_buf        = txb;
  xfer.rx_buf        = rxb;
  xfer.len           = LPC_BB__DRIVER_XFER_SIZE;
  xfer.speed_hz      = LPC_BB__DRIVER_MAX_SPEED;
...
{
  static int            lpc_bb_offline = 0;
  struct lpc_bb_driver *spi            = (struct lpc_bb_driver *) p;
  int                   ready, poll_request, tx_payload, lpc_bb_heartbeat;
  u8 *                  txb;

  ready            = gpio_get_value(spi->rdy_gpio);
  poll_request     = gpio_get_value(spi->prq_gpio);
  lpc_bb_heartbeat = lpc_bb_driver_check_hb(spi->lpc_hb_gpio);
  tx_payload       = tx_write_buffer_offset - SIGNATURE_SIZE;

  if (!lpc_bb_heartbeat)
  {
    if (no_hb_count == 0)
      printk_ratelimited(KERN_INFO "%s() lpc_bb_ heartbeat missing\n", __func__);
  ...
  if (ready && (poll_request || tx_payload))
  {
    ...
    lpc_bb_heartbeat = lpc_bb_driver_check_hb(spi->lpc_hb_gpio);
    if (lpc_bb_heartbeat)
    {  // lpc_bb_ still online
      xfer_repeat = 0;
      lpc_bb_driver_package_parse();  // check any incoming data
    }
    else
    {  // lpc_bb_ offline, re-schedule transfer
      xfer_repeat = 1;
      txb_repeat  = txb;
      printk_ratelimited(KERN_INFO "%s() lpc_bb_ heartbeat missing, re-scheduling transfer\n", __func__);
    }
  }
  else if (!ready)
    busy = 1;
...
exit:
  ...
  {  // we missed enough lpc_bb_ heartbeats to consider lpc_bb_ is offline
    if (!lpc_bb_offline)
    {  // lpc_bb_ went offline now
      lpc_bb_offline = 1;
      printk_ratelimited(KERN_INFO "%s() lpc_bb_ went offline\n", __func__);
    }
  }
  else
  {
    if (lpc_bb_offline)
    {  // lpc_bb_ went online again
      lpc_bb_offline = 0;
      printk_ratelimited(KERN_INFO "%s() lpc_bb_ went online\n", __func__);
    }
  }
...
static ssize_t lpc_bb_driver_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
  ...
  if ((tx_write_buffer_offset + count) > LPC_BB__DRIVER_XFER_SIZE)
  {
    if (count > LPC_BB__DRIVER_XFER_SIZE - 4)
    {
#if LOG_WRITE_STATUS
  ...
  status = copy_from_user((tx_buff + tx_write_buffer * LPC_BB__DRIVER_XFER_SIZE + tx_write_buffer_offset), buf, count);
  if (status == 0)
  {  // copy succeeded
...
static ssize_t lpc_bb_driver_read(struct file *filp, char __user *buf_user, size_t count, loff_t *f_pos)
{
  ...
  second = 0;      // .. and no need for second chunk
  if (rx_buff_tail + count > LPC_BB__DRIVER_RX_BUFF_SIZE)
  {  // read beyond physical buffer end, so split in two chunks
    first  = LPC_BB__DRIVER_RX_BUFF_SIZE - rx_buff_tail;
    second = count - first;
  }
...
static int lpc_bb_driver_probe(struct spi_device *dev)
{
  ...
  rxb = kcalloc(LPC_BB__DRIVER_XFER_SIZE, sizeof(u8), GFP_KERNEL);
  if (!rxb)
  {
  ...
  tx_buff = kcalloc(2 * LPC_BB__DRIVER_XFER_SIZE, sizeof(u8), GFP_KERNEL);
  if (!tx_buff)
  {
  ...
  rx_buff = kcalloc(LPC_BB__DRIVER_RX_BUFF_SIZE, sizeof(u8), GFP_KERNEL);
  if (!rx_buff)
  {
  ...
  {
    dev_err(&dev->dev, "%s: lpc_bb_-hb-gpio in the device tree incorrect\n", __func__);
    ret = -EINVAL;
    goto exit;
  }
  ...
  {
    dev_err(&dev->dev, "%s: lpc_bb_-spare-gpio in the device tree incorrect\n", __func__);
    ret = -EINVAL;
    goto exit;
  }
  ...
  {
    dev_err(&dev->dev, "%s: lpc_bb_-cs-gpio in the device tree incorrect\n", __func__);
    ret = -EINVAL;
    goto exit;
  }
  ...
  dev->mode          = SPI_MODE_1;
  dev->max_speed_hz  = LPC_BB__DRIVER_MAX_SPEED;
  dev->bits_per_word = 8;
  if (spi_setup(dev))
  {
  ...
  /** create the device */
  tmpd                   = device_create(lpc_bb_driver_class, &dev->dev, MKDEV(LPC_BB__DRIVER_DEV_MAJOR, 0), sb, "lpc_bb_driver");
  tx_write_buffer        = 0;
...
static int lpc_bb_driver_remove(struct spi_device *spi)
{
  ...
  device_destroy(lpc_bb_driver_class, MKDEV(LPC_BB__DRIVER_DEV_MAJOR, 0));
  class_destroy(lpc_bb_driver_class);
  unregister_chrdev(LPC_BB__DRIVER_DEV_MAJOR, "spi");
...
static int __init lpc_bb_driver_init(void)
{
  ...
  printk(KERN_INFO "%s() VERSION: 2020-08-19\n", __func__);

  ret = register_chrdev(LPC_BB__DRIVER_DEV_MAJOR, "spi", &lpc_bb_driver_fops);
  if (ret < 0)
