...
const shared = Object.freeze({
    ...
    get colorThemeMatch() {
        return this.colorThemeQuery.matches ? "dark" : "light";
    },
    get theme() {
    ...
    applyColorTheme() {
        // apply media query when theme setting is auto, otherwise use theme setting
        const currentTheme = this.theme;
        let theme;
        if(currentTheme === "auto") {
            theme = this.colorThemeMatch;
        } else {
            theme = currentTheme;
        }
        // update body and pictures
        document.body.dataset.colorTheme = theme;
        const updatePictureEvent = new CustomEvent("update", {detail: {type: "theme", theme: currentTheme}});
    ...
    init() {
        ...
        this.colorThemeQuery.addEventListener("change", () => this.applyColorTheme());
        ...
        for(const anchor of document.querySelectorAll("a[href]")) {
            ...
            if(!("setting" in anchor.dataset) && (anchor.host === location.host)) {
                anchor.setAttribute("href", this.createUrl(anchor.getAttribute("href")));
            }
        }
        // utf
        for(const span of document.querySelectorAll("span[data-utf]")) {
            span.textContent = this.createUtf(span.dataset.utf);
        }
    }
});
