...
static int oleds_fb_setcolreg(unsigned regno, unsigned red, unsigned green, unsigned blue, unsigned transp,
                              struct fb_info *info)
{
  return 0;
}
...
s32 espi_driver_oleds_fb_setup(struct espi_driver *sb)
{
  ...
  info->var.green.length = info->var.transp.length = info->var.blue.length = info->var.red.length
      = info->var.bits_per_pixel;

  info->var.red.offset = info->var.green.offset = info->var.blue.offset = info->var.transp.offset = 0;
