...
#include "playcontroller-converters.h"
#include <string.h>
...
size_t EHC_ComposePlaycontrollerSetupMessageById(enum EHC_PRESET_ID const id, uint8_t const port, enum EHC_RESET_T reset, void *const buffer)
{
  EHC_PresetT const *const pPreset = EHC_GetPresetById(id);
  if (!pPreset)
    return 0;
  return EHC_ComposePlaycontrollerSetupMessageByPreset(pPreset, port, reset, buffer);
}

size_t EHC_ComposePlaycontrollerSetupMessageByPreset(EHC_PresetT const *const pPreset, uint8_t const port, enum EHC_RESET_T reset, void *const buffer)
{
  ...
  CMD_DATA[2]                        = PLAYCONTROLLER_EHC_COMMAND_SET_CONTROL_REGISTER;
  ...
  if (config.hwId != 15)
  {
    ...
    {
      CMD_DATA[2] = PLAYCONTROLLER_EHC_COMMAND_SET_RANGE_MIN | config.ctrlId;
      CMD_DATA[3] = pPreset->rangeMin;
      bytesWritten += writeData(&bufp, sizeof CMD_DATA, &CMD_DATA[0]);

      CMD_DATA[2] = PLAYCONTROLLER_EHC_COMMAND_SET_RANGE_MAX | config.ctrlId;
    ...
    {
      CMD_DATA[2] = PLAYCONTROLLER_EHC_COMMAND_SET_DEAD_ZONES | config.ctrlId;
