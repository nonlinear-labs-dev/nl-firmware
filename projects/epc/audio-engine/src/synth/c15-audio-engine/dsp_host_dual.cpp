...
void dsp_host_dual::init(const uint32_t _samplerate,
                         const uint32_t _polyphony) {
  // BEWARE: we still only support TWO integer multiples of 48000 Hz ("SD": 1 x
  // 48k, "HD": 2 x 48k)
  const uint32_t upsampleFactor =
                     (_samplerate > 96000 ? 96000 : _samplerate) / 48000,
                 upsampleIndex = upsampleFactor - 1;
  if (_samplerate != C15::Config::clock_rates[upsampleIndex][0]) {
    nltools::Log::warning(__PRETTY_FUNCTION__, "invalid sample rate(",
                          _samplerate, "): only 48000 or 96000 Hz allowed!");
  }
  const float samplerate =
      static_cast<float>(C15::Config::clock_rates[upsampleIndex][0]);
  ...
  m_transition_time.init(C15::Properties::SmootherScale::Expon_Env_Time, 1.0f,
                         -20.0f, 0.0f);
  m_transition_time.m_scaled =
      scale(m_transition_time.m_scaling, m_transition_time.m_position);
  ...
  m_poly[0].init(&m_global.m_signals, &m_convert, &m_time, &m_z_layers[0],
                 &m_reference.m_scaled, m_time.m_millisecond,
  ...
  // init poly dsp: exponentiator, feedback pointers
  m_poly[1].init(&m_global.m_signals, &m_convert, &m_time, &m_z_layers[1],
                 &m_reference.m_scaled, m_time.m_millisecond,
                 env_init_gateRelease, samplerate, upsampleFactor);
  // init mono dsp
  ...
  for (auto element : C15::ParameterList) {
    ...
    case C15::Descriptors::ParameterType::Global_Modulateable:
      ...
      case C15::Descriptors::ParameterSignal::Global_Signal:
        ...
        case C15::Descriptors::SmootherClock::Sync:
          m_global.add_copy_sync_id(element.m_ae.m_smoother.m_index,
                                    element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Audio:
          m_global.add_copy_audio_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Fast:
          m_global.add_copy_fast_id(element.m_ae.m_smoother.m_index,
                                    element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Slow:
          m_global.add_copy_slow_id(element.m_ae.m_smoother.m_index,
                                    element.m_ae.m_signal.m_index);
          break;
        }
        break;
    ...
    case C15::Descriptors::ParameterType::Local_Modulateable:
      ...
      case C15::Descriptors::ParameterSignal::Quasipoly_Signal:
        switch (element.m_ae.m_smoother.m_clock) {
        case C15::Descriptors::SmootherClock::Sync:
          m_poly[0].add_copy_sync_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_poly[1].add_copy_sync_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Audio:
          m_poly[0].add_copy_audio_id(element.m_ae.m_smoother.m_index,
                                      element.m_ae.m_signal.m_index);
          m_poly[1].add_copy_audio_id(element.m_ae.m_smoother.m_index,
                                      element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Fast:
          m_poly[0].add_copy_fast_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_poly[1].add_copy_fast_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Slow:
          m_poly[0].add_copy_slow_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_poly[1].add_copy_slow_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        }
        break;
      case C15::Descriptors::ParameterSignal::Mono_Signal:
        switch (element.m_ae.m_smoother.m_clock) {
        case C15::Descriptors::SmootherClock::Sync:
          m_mono[0].add_copy_sync_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_mono[1].add_copy_sync_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Audio:
          m_mono[0].add_copy_audio_id(element.m_ae.m_smoother.m_index,
                                      element.m_ae.m_signal.m_index);
          m_mono[1].add_copy_audio_id(element.m_ae.m_smoother.m_index,
                                      element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Fast:
          m_mono[0].add_copy_fast_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_mono[1].add_copy_fast_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        case C15::Descriptors::SmootherClock::Slow:
          m_mono[0].add_copy_slow_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          m_mono[1].add_copy_slow_id(element.m_ae.m_smoother.m_index,
                                     element.m_ae.m_signal.m_index);
          break;
        }
        break;
...
void dsp_host_dual::logStatus() {
  if (LOG_ENGINE_STATUS) {
    ...
    nltools::Log::info("-clock(index:", m_clock.m_index,
                       ", fast:", m_clock.m_fast, ", slow:", m_clock.m_slow,
                       ")");
    nltools::Log::info("-output(left:", m_mainOut_L, ", right:", m_mainOut_R,
                       ", mute:", m_fade.getValue(), ")");
    nltools::Log::info("-dsp(dx:", m_time.m_sample_inc,
                       ", ms:", m_time.m_millisecond, ")");
  ...
  if (m_glitch_suppression) {
    ...
    if (LOG_RECALL) {
      nltools::Log::info(__PRETTY_FUNCTION__,
                         m_fade.getNumMuteSamplesRendered(),
                         "mute samples rendered.");
    }
...
void dsp_host_dual::globalParChg(
    const uint32_t _id, const nltools::msg::HWAmountChangedMessage &_msg) {
  auto param = m_params.get_hw_amt(_id);
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    if (LOG_EDITS) {
      nltools::Log::info("ha_edit(srcId:", param->m_sourceId, ", amtId:", _id,
                         ", pos:", param->m_position, ")");
    }
  }
}

void dsp_host_dual::globalParChg(
    const uint32_t _id, const nltools::msg::MacroControlChangedMessage &_msg) {
  ...
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    ...
    param->m_unclipped =
        param->m_position; // fixing #2023: unclipped always up-to-date
...
void dsp_host_dual::globalParChg(
    const uint32_t _id,
    const nltools::msg::ModulateableParameterChangedMessage &_msg) {
  ...
  if (param->update_position(
          param->depolarize(static_cast<float>(_msg.controlPosition)))) {
    param->update_modulation_aspects(m_params.get_macro(macroId)->m_position);
    param->m_scaled =
        scale(param->m_scaling, param->polarize(param->m_position));
    if (LOG_EDITS) {
      nltools::Log::info("global_target_edit(pos:", param->m_position,
                         ", val:", param->m_scaled, ")");
      if (aspect_update) {
        nltools::Log::info("global_target_edit(mc:", macroId,
                           ", amt:", param->m_amount, ")");
      }
    }
...
void dsp_host_dual::globalParChg(
    const uint32_t _id,
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
  auto param = m_params.get_global_direct(_id);
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    param->m_scaled = scale(param->m_scaling, param->m_position);
    if (LOG_EDITS) {
      nltools::Log::info("global_direct_edit(pos:", param->m_position,
                         ", val:", param->m_scaled, ")");
    }
    if (_id == static_cast<uint32_t>(
                   C15::Parameters::Global_Unmodulateables::Scale_Base_Key)) {
...
void dsp_host_dual::globalTimeChg(
    const uint32_t _id,
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
...
void dsp_host_dual::localParChg(
    const uint32_t _id,
    const nltools::msg::ModulateableParameterChangedMessage &_msg) {
  const uint32_t layerId = getLayerId(_msg.voiceGroup),
                 macroId = getMacroId(_msg.sourceMacro);
  ...
  if (param->update_position(
    ...
    param->m_scaled =
        scale(param->m_scaling, param->polarize(param->m_position));
    if (LOG_EDITS) {
      nltools::Log::info("local_target_edit(layer:", layerId,
                         ", pos:", param->m_position, ", val:", param->m_scaled,
                         ")");
      if (aspect_update) {
        nltools::Log::info("local_target_edit(layer:", layerId,
                           ", mc:", macroId, ", amt:", param->m_amount, ")");
      }
    }
    ...
    param->update_modulation_aspects(m_params.get_macro(macroId)->m_position);
    if (LOG_EDITS) {
      nltools::Log::info("local_target_edit(layer:", layerId, ", mc:", macroId,
                         ", amt:", param->m_amount, ")");
    }
  }
}
...
void dsp_host_dual::localParChg(
  ...
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    param->m_scaled = scale(param->m_scaling, param->m_position);
    if (LOG_EDITS) {
      nltools::Log::info("local_direct_edit(layer:", layerId,
                         ", pos:", param->m_position, ", val:", param->m_scaled,
                         ")");
    }
...
DSPInterface::OutputResetEventSource dsp_host_dual::localUnisonVoicesChg(
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
  const uint32_t layerId = getLayerId(_msg.voiceGroup);
  ...
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    if (LOG_EDITS) {
      nltools::Log::info("unison_voices_edit(layer:", layerId,
                         ", pos:", param->m_position, ")");
    }
    ...
    if (m_layer_mode != LayerMode::Split) {
      ...
      case OutputResetEventSource::Local_Both:
        return isPartI ? OutputResetEventSource::Local_I
                       : OutputResetEventSource::Local_II;
      }
    }
  }
...
DSPInterface::OutputResetEventSource dsp_host_dual::localMonoEnableChg(
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
  const uint32_t layerId = getLayerId(_msg.voiceGroup);
  auto param = m_params.get_local_mono_enable(getLayer(_msg.voiceGroup));
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    if (LOG_EDITS) {
      nltools::Log::info("mono_enable_edit(layer:", layerId,
                         ", pos:", param->m_position, ")");
    }
    const OutputResetEventSource outputEvent = determineOutputEventSource(
        areKeysPressed(fromType(m_layer_mode)), m_layer_mode);
...
void dsp_host_dual::localMonoPriorityChg(
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
  const uint32_t layerId = getLayerId(_msg.voiceGroup);
  auto param = m_params.get_local_mono_priority(getLayer(_msg.voiceGroup));
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    if (LOG_EDITS) {
      nltools::Log::info("mono_priority_edit(layer:", layerId,
                         ", pos:", param->m_position, ")");
    }
    param->m_scaled = scale(param->m_scaling, param->m_position);
...
void dsp_host_dual::localMonoLegatoChg(
    const nltools::msg::UnmodulateableParameterChangedMessage &_msg) {
  const uint32_t layerId = getLayerId(_msg.voiceGroup);
  auto param = m_params.get_local_mono_legato(getLayer(_msg.voiceGroup));
  if (param->update_position(static_cast<float>(_msg.controlPosition))) {
    if (LOG_EDITS) {
      nltools::Log::info("mono_leagto_edit(layer:", layerId,
                         ", pos:", param->m_position, ")");
    }
    param->m_scaled = scale(param->m_scaling, param->m_position);
...
void dsp_host_dual::onSettingEditTime(const float _position) {
  ...
  if (m_edit_time.update_position(_position < 0.0f   ? 0.0f
                                  : _position > 1.0f ? 1.0f
                                                     : _position)) {
    if (LOG_SETTINGS) {
      nltools::Log::info("edit_time(pos:", m_edit_time.m_position,
                         ", raw:", _position, ")");
    }
    updateTime(&m_edit_time.m_dx,
               scale(m_edit_time.m_scaling, m_edit_time.m_position));
  }
}

void dsp_host_dual::onSettingTransitionTime(const float _position) {
  // inconvenient clamping of odd position range ...
  if (m_transition_time.update_position(_position < 0.0f   ? 0.0f
                                        : _position > 1.0f ? 1.0f
                                                           : _position)) {
    if (LOG_SETTINGS) {
      nltools::Log::info("transition_time(pos:", m_transition_time.m_position,
                         ", raw:", _position, ")");
    }
    updateTime(&m_transition_time.m_dx, scale(m_transition_time.m_scaling,
                                              m_transition_time.m_position));
  }
}
...
void dsp_host_dual::onSettingTuneReference(const float _position) {
  // inconvenient clamping of odd position range ...
  if (m_reference.update_position(_position < 0.0f   ? 0.0f
                                  : _position > 1.0f ? 1.0f
                                                     : _position)) {
    m_reference.m_scaled = scale(m_reference.m_scaling, m_reference.m_position);
    m_global.update_tone_frequency(m_reference.m_scaled);
...
DSPInterface::OutputResetEventSource
dsp_host_dual::onSettingInitialSinglePreset() {
  // todo: rework process
  if (LOG_RECALL) {
  ...
  for (uint32_t layerId = 0; layerId < m_params.m_layer_count; layerId++) {
    ...
    for (uint32_t i = 0; i < m_params.m_layer[0].m_target_count; i++) {
      ...
      param->update_modulation_aspects(
          m_params.get_macro(static_cast<uint32_t>(param->m_source))
              ->m_position);
      param->m_scaled =
          scale(param->m_scaling, param->polarize(param->m_position));
      localTransition(layerId, param, m_transition_time.m_dx);
...
void dsp_host_dual::render() {
  ...
  // cross-feedback
  const float left[2] = {m_poly[0].m_send_self_l + m_poly[1].m_send_other_l +
                             m_z_layers[1].m_fx_to_other_l,
                         m_poly[0].m_send_other_l + m_poly[1].m_send_self_l +
                             m_z_layers[0].m_fx_to_other_l};
  const float right[2] = {m_poly[0].m_send_self_r + m_poly[1].m_send_other_r +
                              m_z_layers[1].m_fx_to_other_r,
                          m_poly[0].m_send_other_r + m_poly[1].m_send_self_r +
                              m_z_layers[0].m_fx_to_other_r};
  m_mono[0].render_audio(
      left[0], right[0], m_poly[0].getVoiceGroupVolume(),
      m_global.m_signals.get(C15::Signals::Global_Signals::Master_FX_I_to_II));
  m_mono[1].render_audio(
      left[1], right[1], m_poly[1].getVoiceGroupVolume(),
  ...
  // clip
  m_global.render_audio(m_mono[0].m_out_l, m_mono[0].m_out_r, m_mono[1].m_out_l,
                        m_mono[1].m_out_r);
...
dsp_host_dual::HWSourceValues dsp_host_dual::getHWSourceValues() const {
  ...
  std::transform(m_params.m_global.m_source,
                 m_params.m_global.m_source + ret.size(), ret.begin(),
                 [](const auto &a) { return a.m_position; });
  return ret;
}

C15::Properties::HW_Return_Behavior
dsp_host_dual::getBehavior(const ReturnMode _mode) {
  switch (_mode) {
...
C15::Properties::HW_Return_Behavior
dsp_host_dual::getBehavior(const RibbonReturnMode _mode) {
  switch (_mode) {
...
C15::Parameters::Macro_Controls
dsp_host_dual::getMacro(const MacroControls _mc) {
  static_assert(static_cast<int>(MacroControls::NONE) ==
                static_cast<int>(C15::Parameters::Macro_Controls::None));
  static_assert(static_cast<int>(MacroControls::MC1) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_A));
  static_assert(static_cast<int>(MacroControls::MC2) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_B));
  static_assert(static_cast<int>(MacroControls::MC3) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_C));
  static_assert(static_cast<int>(MacroControls::MC4) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_D));
  static_assert(static_cast<int>(MacroControls::MC5) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_E));
  static_assert(static_cast<int>(MacroControls::MC6) ==
                static_cast<int>(C15::Parameters::Macro_Controls::MC_F));
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::NONE) ==
      C15::Parameters::Macro_Controls::None);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC1) ==
      C15::Parameters::Macro_Controls::MC_A);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC2) ==
      C15::Parameters::Macro_Controls::MC_B);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC3) ==
      C15::Parameters::Macro_Controls::MC_C);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC4) ==
      C15::Parameters::Macro_Controls::MC_D);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC5) ==
      C15::Parameters::Macro_Controls::MC_E);
  static_assert(
      static_cast<C15::Parameters::Macro_Controls>(MacroControls::MC6) ==
...
void dsp_host_dual::keyDownTraversal(const uint32_t _note, const float _vel,
  ...
  if (LOG_KEYS) {
    nltools::Log::info("key_down(src:", _inputSourceId, ", pos:", _note,
                       ", vel:", _vel, ", unison:", m_alloc.m_unison, ")");
  }
  ...
  for (auto event = m_alloc.m_traversal.first(); m_alloc.m_traversal.running();
       event = m_alloc.m_traversal.next()) {
    if (m_poly[event->m_localIndex].keyDown(event)) {
      // mono legato
      m_mono[event->m_localIndex].keyDown(event);
    }
    if (LOG_KEYS_POLY) {
      nltools::Log::info("key_down_poly(group:", event->m_localIndex,
                         "voice:", event->m_voiceId,
                         ", unisonIndex:", event->m_unisonIndex,
                         ", stolen:", event->m_stolen, ", tune:", event->m_tune,
                         ", velocity:", event->m_velocity, ")");
      nltools::Log::info("key_details(active:", event->m_active,
                         ", trigger_env:", event->m_trigger_env,
...
void dsp_host_dual::keyUpTraversal(const uint32_t _note, const float _vel,
                                   const uint32_t _inputSourceId) {
  if (LOG_KEYS) {
    nltools::Log::info("key_up(src:", _inputSourceId, ", pos:", _note,
                       ", vel:", _vel, ", unison:", m_alloc.m_unison, ")");
  }
  for (auto event = m_alloc.m_traversal.first(); m_alloc.m_traversal.running();
       event = m_alloc.m_traversal.next()) {
    ...
    if (LOG_KEYS_POLY) {
      nltools::Log::info("key_up_poly(group:", event->m_localIndex,
                         "voice:", event->m_voiceId, ", tune:", event->m_tune,
                         ", velocity:", event->m_velocity, ")");
      nltools::Log::info("key_details(active:", event->m_active,
                         ", trigger_env:", event->m_trigger_env,
                         ", trigger_glide:", event->m_trigger_glide, ")");
...
float dsp_host_dual::scale(const Scale_Aspect _scl, float _value) {
  ...
  case C15::Properties::SmootherScale::Linear2Sections:
  {
    ...
    {
      result = 1 + 11 * (_value - lim) / (1.0f - lim);
    }
    else
    {
      result = -1 + 11 * (_value + lim) / (1.0f - lim);
  ...
  case C15::Properties::SmootherScale::S_Curve:
    ...
    result = _scl.m_offset +
             (_scl.m_factor *
              ((_value * _value * _value * -0.25f) + (_value * 0.75f) + 0.5f));
    break;
  ...
  case C15::Properties::SmootherScale::Expon_Shaper_Drive:
    result = (m_convert.eval_level(_value * _scl.m_factor) * _scl.m_offset) -
             _scl.m_offset;
    break;
  ...
  case C15::Properties::SmootherScale::Expon_Env_Time:
    result = m_convert.eval_time((_value * _scl.m_factor * 104.0781f) +
                                 _scl.m_offset);
    break;
  }
  return result;
...
void dsp_host_dual::updateTime(Time_Aspect *_param, const float _ms) {
  ...
  if (LOG_TIMES) {
    nltools::Log::info("time_update(ms:", _ms, ", dx:", m_time.m_dx_audio,
                       m_time.m_dx_fast, m_time.m_dx_slow, ")");
  }
}

void dsp_host_dual::hwModChain(HW_Src_Param *_src, const uint32_t _id,
                               const float _inc) {
  if (LOG_HW) {
    nltools::Log::info("hw_move(id:", _id, ", pos:", _src->m_position, ")");
  }
  if (_src->m_behavior == C15::Properties::HW_Return_Behavior::Stay) {
    for (uint32_t macroId = 1; macroId < m_params.m_global.m_macro_count;
         macroId++) {
      ...
      if (amount->m_position != 0.0f) {
        ...
        macro->m_unclipped = macro->m_position =
            _src->m_position; // fixing #2023: unclipped always up-to-date
        if (m_layer_mode == LayerMode::Single) {
          if (LOG_HW) {
            nltools::Log::info("hw_nonreturn_to_mc(mode: single, srcId:", _id,
                               ", mcId:", macroId, ", amtId:", amountIndex,
                               ", amount:", amount->m_position,
                               ", value:", macro->m_position, ")");
          }
          localModChain(macro);
        } else {
          if (LOG_HW) {
            nltools::Log::info("hw_nonreturn_to_mc(mode: dual, srcId:", _id,
                               ", mcId:", macroId, ", amtId:", amountIndex,
                               ", amount:", amount->m_position,
                               ", value:", macro->m_position, ")");
          }
          for (uint32_t layerId = 0; layerId < m_params.m_layer_count;
               layerId++) {
      ...
      if (amount->m_position != 0.0f) {
        ...
        if (amount->m_position != 0.0f) {
          ...
          if (macro->m_position != clipped) {
            ...
            if (m_layer_mode == LayerMode::Single) {
              if (LOG_HW) {
                nltools::Log::info("hw_return_to_mc(mode: single, srcId:", _id,
                                   ", mcId:", macroId, ", amtId:", amountIndex,
                                   ", amount:", amount->m_position,
                                   ", value:", macro->m_position, ")");
              }
              ...
              if (LOG_HW) {
                nltools::Log::info("hw_return_to_mc(mode: dual, srcId:", _id,
                                   ", mcId:", macroId, ", amtId:", amountIndex,
                                   ", amount:", amount->m_position,
                                   ", value:", macro->m_position, ")");
              }
...
void dsp_host_dual::globalModChain(Macro_Param *_mc) {
  for (auto param = m_params.globalChainFirst(_mc->m_id); param;
       param = m_params.globalChainNext()) {
    if (param->modulate(_mc->m_position)) {
      const float clipped = param->m_unclipped < 0.0f   ? 0.0f
                            : param->m_unclipped > 1.0f ? 1.0f
                                                        : param->m_unclipped;
...
void dsp_host_dual::localModChain(Macro_Param *_mc) {
  for (auto param = m_params.localChainFirst(0, _mc->m_id); param;
...
void dsp_host_dual::localModChain(const uint32_t _layer, Macro_Param *_mc) {
  for (auto param = m_params.localChainFirst(_layer, _mc->m_id); param;
...
void dsp_host_dual::globalTransition(const Target_Param *_param,
                                     const Time_Aspect _time) {
  ...
  if (LOG_TRANSITIONS && !fail) {
    nltools::Log::info("global_transition(section:",
                       static_cast<int>(_param->m_render.m_section),
                       ", index:", _param->m_render.m_index,
                       ", clock:", static_cast<int>(_param->m_render.m_clock),
                       ", dx:", dx, ", dest:", dest, ")");
  } else if (LOG_FAIL && fail) {
    nltools::Log::warning(
        __PRETTY_FUNCTION__, "failed to start global_transition(section:",
        static_cast<int>(_param->m_render.m_section),
        ", index:", _param->m_render.m_index,
        ", clock:", static_cast<int>(_param->m_render.m_clock), ")");
  }
...
void dsp_host_dual::globalTransition(const Direct_Param *_param,
                                     const Time_Aspect _time) {
  float dx = 0.0f, dest = _param->m_scaled;
  bool fail = false;
  switch (_param->m_render.m_section) {
...
void dsp_host_dual::localTransition(const uint32_t _layer,
                                    const Direct_Param *_param,
                                    const Time_Aspect _time) {
  float dx = 0.0f, dest = _param->m_scaled;
  bool fail = false;
  switch (_param->m_render.m_section) {
  ...
  if (LOG_TRANSITIONS && !fail) {
    nltools::Log::info("local_transition(layer:", _layer, ", section:",
                       static_cast<int>(_param->m_render.m_section),
                       ", index:", _param->m_render.m_index,
                       ", clock:", static_cast<int>(_param->m_render.m_clock),
                       ", dx:", dx, ", dest:", dest, ")");
  ...
  if (LOG_FAIL && fail) {
    nltools::Log::warning(
        __PRETTY_FUNCTION__, "failed to start local_transition(layer:", _layer,
...
bool dsp_host_dual::evalPolyChg(
  ...
  bool unisonChanged = unison_voices->update_position(
      static_cast<float>(_unisonVoices.controlPosition));
  auto mono_enable = m_params.get_local_mono_enable(_layerId);
  bool monoChanged = mono_enable->update_position(
      static_cast<float>(_monoEnable.controlPosition));
...
void dsp_host_dual::evalVoiceFadeChg(const uint32_t _layer) {
  ...
  const uint32_t edgeCaseHackKey[2] = {C15::Config::physical_key_to,
                                       C15::Config::physical_key_from},
                 edgeCaseHackRemap[2] = {C15::Config::virtual_key_to,
                                         C15::Config::virtual_key_from};
  ...
  uint32_t from =
      static_cast<uint32_t>(
          m_params
              .get_local_direct(
                  _layer,
                  static_cast<uint32_t>(C15::Parameters::Local_Unmodulateables::
                                            Voice_Grp_Fade_From))
              ->m_scaled) +
      C15::Config::physical_key_from;
  ...
  const float range =
      m_params
          .get_local_direct(
              _layer,
              static_cast<uint32_t>(
                  C15::Parameters::Local_Unmodulateables::Voice_Grp_Fade_Range))
          ->m_scaled;
...
DSPInterface::OutputResetEventSource
dsp_host_dual::determineOutputEventSource(const bool _detected,
...
dsp_host_dual::recallSingle(const nltools::msg::SinglePresetMessage &_msg) {
  ...
  auto polyChanged =
      evalPolyChg(C15::Properties::LayerId::I, msg->unison.unisonVoices,
                  msg->mono.monoEnable);
  // reset detection
  const bool internalReset = layerChanged || polyChanged;
  const bool externalReset =
      internalReset && areKeysPressed(fromType(oldLayerMode));
  const OutputResetEventSource outputEvent =
      determineOutputEventSource(externalReset, oldLayerMode);
  if (internalReset) {
    if (LOG_RESET) {
      nltools::Log::info("recall single voice reset");
    }
    ...
    m_alloc.setUnison(
        0,
        m_params.get_local_unison_voices(C15::Properties::LayerId::I)
            ->m_position,
        oldLayerMode, m_layer_mode);
    m_alloc.setMonoEnable(
        0,
        m_params.get_local_mono_enable(C15::Properties::LayerId::I)->m_position,
        m_layer_mode);
    const uint32_t uVoice = m_alloc.m_unison - 1;
    for (uint32_t layerId = 0; layerId < m_params.m_layer_count; layerId++) {
      m_poly[layerId].resetEnvelopes();
  ...
  if (LOG_RECALL) {
    nltools::Log::info(
        "recall: global params (modulateables/unmodulateables):");
  }
  ...
  for (uint32_t i = 0; i < m_params.m_global.m_direct_count; i++) {
    ...
    if (i == static_cast<uint32_t>(
                 C15::Parameters::Global_Unmodulateables::Scale_Base_Key)) {
      m_global.start_base_key(m_transition_time.m_dx.m_dx_slow,
                              param->m_scaled);
    } else {
      globalTransition(param, m_transition_time.m_dx);
    }
...
dsp_host_dual::recallSplit(const nltools::msg::SplitPresetMessage &_msg) {
  ...
  const bool externalReset =
      layerChanged && areKeysPressed(fromType(oldLayerMode));
  for (uint32_t layerId = 0; layerId < m_params.m_layer_count; layerId++) {
    const auto layer = static_cast<C15::Properties::LayerId>(layerId);

    // update unison and mono groups
    auto polyChanged = evalPolyChg(layer, msg->unison[layerId].unisonVoices,
                                   msg->mono[layerId].monoEnable);
    ...
    if ((layerChanged || polyChanged)) {
      ...
      m_alloc.m_internal_keys.m_local[layerId] =
          0; // reset all pressed keys in part
      m_alloc.setUnison(layerId,
                        m_params.get_local_unison_voices(layer)->m_position,
                        oldLayerMode, m_layer_mode);
      m_alloc.setMonoEnable(layerId,
                            m_params.get_local_mono_enable(layer)->m_position,
                            m_layer_mode);
      const uint32_t uVoice = m_alloc.m_unison - 1;
      m_poly[layerId].resetEnvelopes();
      m_poly[layerId].m_uVoice = uVoice;
  ...
  // change)
  return m_alloc.m_internal_keys.pressedLocalKeys(internalReset[0],
                                                  internalReset[1]);
}
...
void dsp_host_dual::globalParRcl(
  ...
  if (element.m_param.m_type ==
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall HW Source(id:", _param.id, ")");
  }
}
void dsp_host_dual::globalParRcl(
  ...
  if (element.m_param.m_type ==
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall HW Amount(id:", _param.id, ")");
  }
}
...
void dsp_host_dual::globalParRcl(
  ...
  if (element.m_param.m_type ==
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall Macro(id:", _param.id, ")");
  }
}
...
void dsp_host_dual::globalParRcl(
  ...
  if(element.m_param.m_type == C15::Descriptors::ParameterType::Global_Modulateable)
  {
    ...
    param->update_position(
        param->depolarize(static_cast<float>(_param.controlPosition)));
    param->m_scaled =
        scale(param->m_scaling, param->polarize(param->m_position));
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall Global Modulateable(id:", _param.id,
                          ")");
  }
}
...
void dsp_host_dual::globalParRcl(
  ...
  if(element.m_param.m_type == C15::Descriptors::ParameterType::Global_Unmodulateable)
  {
    ...
    nltools::Log::warning(
        __PRETTY_FUNCTION__,
        "failed to recall Global Unmodulateable(id:", _param.id, ")");
  }
}
...
void dsp_host_dual::globalParRcl(
  ...
  if(element.m_param.m_type == C15::Descriptors::ParameterType::Global_Unmodulateable)
  {
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall GlobalParameter(id:", _param.id,
                          ")");
  }
}
...
void dsp_host_dual::globalTimeRcl(
  ...
  if (element.m_param.m_type == C15::Descriptors::ParameterType::Macro_Time) {
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall Macro Time(id:", _param.id, ")");
  }
}
void dsp_host_dual::localParRcl(
  ...
  if(element.m_param.m_type == C15::Descriptors::ParameterType::Local_Modulateable)
  {
    ...
    param->m_scaled =
        scale(param->m_scaling, param->polarize(param->m_position));
    const uint32_t macroId = getMacroId(_param.mc);
    m_params.m_layer[_layerId].m_assignment.reassign(element.m_param.m_index,
                                                     macroId);
    param->update_modulation_aspects(m_params.get_macro(macroId)->m_position);
  } else if (LOG_FAIL) {
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall Local Target(id:", _param.id, ")");
  }
}
...
void dsp_host_dual::localParRcl(
  ...
  if(element.m_param.m_type == C15::Descriptors::ParameterType::Local_Unmodulateable)
  {
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__,
                          "failed to recall Local Direct(id:", _param.id, ")");
  }
}
...
void dsp_host_dual::localPolyRcl(
  ...
  if (_va_update) {
    ...
    m_alloc.setMonoPriority(_layerId,
                            m_params.get_local_mono_priority(layerId)->m_scaled,
                            m_layer_mode);
    m_alloc.setMonoLegato(_layerId,
                          m_params.get_local_mono_legato(layerId)->m_scaled,
                          m_layer_mode);
  }
}
...
void dsp_host_dual::debugLevels() {
  nltools::Log::info(
      "MasterLevel:",
      m_params
          .get_global_direct(static_cast<uint32_t>(
              C15::Parameters::Global_Modulateables::Master_Volume))
          ->m_scaled);
  nltools::Log::info(
      "VoiceGroupLevel[I]:",
      m_params
          .get_local_target(
              0, static_cast<uint32_t>(
                     C15::Parameters::Local_Modulateables::Voice_Grp_Volume))
          ->m_scaled);
  nltools::Log::info(
      "VoiceGroupLevel[II]:",
      m_params
...
void dsp_host_dual::onHWChanged(HardwareSource id, float value,
...
C15::Properties::HW_Return_Behavior
dsp_host_dual::getBehaviour(HardwareSource id) {
  if (id != HardwareSource::NONE) {
    auto idx = static_cast<int>(id);
...
VoiceGroup dsp_host_dual::getSplitPartForKeyUp(int key, InputEventSource from) {
  const auto allocID =
      m_alloc.getSplitPartForKeyUp(key, getInputSourceId(from));
  return getVoiceGroupFromAllocatorId(allocID);
}
...
void dsp_host_dual::registerNonLocalKeyAssignment(const int note,
                                                  VoiceGroup part) {
...
void dsp_host_dual::onKeyDown(const int note, float velocity,
                              InputEventSource from) {
  ...
  {
    nltools::Log::warning(__PRETTY_FUNCTION__, "keyDown(src:", inputSourceId,
                          ", pos:", note, ") failed!");
  }
}

void dsp_host_dual::onKeyUp(const int note, float velocity,
                            InputEventSource from) {
  // InputEvent can be singular (TCD or Primary) or separate (Primary or
  // Secondary or Both) Secondary only exists in Split Sounds, so the final
  // sourceId should be either TCD (0) or Primary (1)
  ...
  if (valid) {
    ...
    nltools::Log::warning(__PRETTY_FUNCTION__, "keyUp(src:", inputSourceId,
                          ", pos:", note, ") failed!");
  }
}

void dsp_host_dual::onKeyDownSplit(const int note, float velocity,
                                   VoiceGroup part,
  ...
  if (m_layer_mode == LayerMode::Split) {
    switch (part) {
    case VoiceGroup::I: // applies to Part I only
      valid = m_alloc.onSplitKeyDown(note, velocity, inputSourceId,
                                     AllocatorId::Local_I);
      break;
    case VoiceGroup::II: // applies to Part II only
      valid = m_alloc.onSplitKeyDown(note, velocity, inputSourceId,
                                     AllocatorId::Local_II);
      break;
    case VoiceGroup::Global: // applies to both Parts I, II at once
      valid = m_alloc.onSplitKeyDown(note, velocity, inputSourceId,
                                     AllocatorId::Local_Both);
      break;
    default:
    case VoiceGroup::NumGroups:
    case VoiceGroup::Invalid:
...
void dsp_host_dual::onKeyUpSplit(const int note, float velocity,
                                 VoiceGroup part,
                                 DSPInterface::InputEventSource from) {
  const uint32_t inputSourceId = getInputSourceId(from);
  bool valid = false;
  if (m_layer_mode == LayerMode::Split) {
    switch (part) {
    case VoiceGroup::I: // applies to Part I only
      valid = m_alloc.onSplitKeyUp(note, velocity, inputSourceId,
                                   AllocatorId::Local_I);
      break;
    case VoiceGroup::II: // applies to Part II only
      valid = m_alloc.onSplitKeyUp(note, velocity, inputSourceId,
...
float dsp_host_dual::getReturnValueFor(HardwareSource hwid) { return 0; }
...
bool dsp_host_dual::areKeysPressed(SoundType _current) {
  ...
  case SoundType::Split:
    return (m_alloc.m_internal_keys.m_local[0] +
            m_alloc.m_internal_keys.m_local[1]) > 0;
  }
  return false;
}
