...
{
  template<CC ID>
  static inline constexpr uint32_t getCC()
  {
...
struct FullCCRange
{
  ...
  static uint16_t encodeUnipolarMidiValue(float _controlPosition)
  {
    ...
    {
      return topOffset + static_cast<uint16_t>(static_cast<float>(topRange) * _controlPosition);
    }
    else
    {
      return static_cast<uint16_t>(static_cast<float>(fullRange) * _controlPosition);
    }
  }
