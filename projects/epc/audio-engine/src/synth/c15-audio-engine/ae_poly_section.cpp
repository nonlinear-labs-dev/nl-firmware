...
void PolySection::init(GlobalSignals *_globalsignals, exponentiator *_convert,
                       Engine::Handle::Time_Handle *_time,
                       LayerSignalCollection *_z_self, float *_reference,
                       const float _ms, const float _gateRelease,
  ...
  m_gain_curve_size =
      static_cast<uint32_t>(POTENTIAL_SETTING_GAIN_CURVE_TIME_MS * _ms);
  ...
  for (uint32_t curve_idx = 0; curve_idx < m_gain_curve_size; curve_idx++) {
    const float curve_phase =
                    NlToolbox::Constants::pi *
                    ((curve_norm * static_cast<float>(curve_idx)) - 0.5f),
                curve_val = std::sin(curve_phase);
    m_gain_curve_buffer[curve_idx] =
        0.5f + (0.5f * curve_val); // sine-based crossfade
  }
#endif
}

void PolySection::add_copy_sync_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
  m_smoothers.m_copy_sync.add_copy_id(_smootherId, _signalId);
}

void PolySection::add_copy_audio_id(const uint32_t _smootherId,
                                    const uint32_t _signalId) {
  m_smoothers.m_copy_audio.add_copy_id(_smootherId, _signalId);
}

void PolySection::add_copy_fast_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
  m_smoothers.m_copy_fast.add_copy_id(_smootherId, _signalId);
}

void PolySection::add_copy_slow_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
...
void PolySection::start_audio(const uint32_t _id, const float _dx,
                              const float _dest) {
  m_smoothers.start_audio(_id, _dx, _dest);
}

void PolySection::start_fast(const uint32_t _id, const float _dx,
                             const float _dest) {
  m_smoothers.start_fast(_id, _dx, _dest);
}

void PolySection::start_slow(const uint32_t _id, const float _dx,
                             const float _dest) {
...
void PolySection::render_audio(const float _mute) {
  ...
  for (uint32_t i = 0; i < traversal->m_length; i++) {
    m_signals.set_mono(traversal->m_signalId[i],
  ...
  m_combfilter.apply(m_signals, m_soundgenerator.m_out_A,
                     m_soundgenerator.m_out_B);
  m_svfilter.apply(m_signals, m_soundgenerator.m_out_A,
                   m_soundgenerator.m_out_B, m_combfilter.m_out);
  m_outputmixer.combine(m_signals, m_voice_level, m_soundgenerator.m_out_A,
                        m_soundgenerator.m_out_B, m_combfilter.m_out,
                        m_svfilter.m_out);
  ...
  // as a separate voice loop without control flow branches, this loop should be
  // SIMD-compliant
  for (uint32_t v = 0; v < m_voices; v++) {
    const uint32_t v_index = m_gain_curve_index[v];
    const float v_fade = m_gain_curve_buffer[v_index];
    tmp_voice_level[v] =
        ((1.0f - v_fade) * m_voice_level_old[v]) + (v_fade * m_voice_level[v]);
  ...
  m_svfilter.apply(m_signals, m_soundgenerator.m_out_A,
                   m_soundgenerator.m_out_B, m_combfilter.m_out);
  m_outputmixer.combine(m_signals, tmp_voice_level, m_soundgenerator.m_out_A,
                        m_soundgenerator.m_out_B, m_combfilter.m_out,
                        m_svfilter.m_out);
#endif
  ...
  // eval sends
  const float send_other =
                  m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_To_FX),
              send_self = 1.0f - send_other;
...
void PolySection::render_feedback(const LayerSignalCollection &_z_other) {
  ...
  m_soundgenerator.m_feedback_phase_A = m_soundgenerator.m_feedback_phase_B =
      m_feedbackmixer.m_out;
}
...
void PolySection::render_fast() {
  ...
  for (uint32_t i = 0; i < traversal->m_length; i++) {
    m_signals.set_mono(traversal->m_signalId[i],
                       m_smoothers.get_fast(traversal->m_smootherId[i]));
  }
...
void PolySection::render_slow() {
  ...
  m_mono_glide.timeUpdate(
      0, m_time->eval_ms(
             3, m_smoothers.get(C15::Smoothers::Poly_Slow::Mono_Grp_Glide)));
  m_mono_glide.render();
  auto traversal = &m_smoothers.m_copy_slow;
  for (uint32_t i = 0; i < traversal->m_length; i++) {
    m_signals.set_mono(traversal->m_signalId[i],
                       m_smoothers.get_slow(traversal->m_smootherId[i]));
  }
...
void PolySection::onBeforeKeyDown() {
...
bool PolySection::keyDown(PolyKeyEvent *_event) {
  const bool retrigger_mono = (m_key_active == 0),
             rstA = _event->m_trigger_env &&
                    static_cast<bool>(m_signals.get(
                        C15::Signals::Quasipoly_Signals::Osc_A_Reset)),
             rstB = _event->m_trigger_env &&
                    static_cast<bool>(m_signals.get(
                        C15::Signals::Quasipoly_Signals::Osc_B_Reset));
...
#endif
    startEnvelopes(_event->m_voiceId, m_note_pitch[_event->m_voiceId],
                   _event->m_velocity);
  }
  // process signals after starting envelopes
  postProcess_poly_key(_event->m_voiceId);
  setSlowFilterCoefs(_event->m_voiceId);
  // set comb delay smoother after pitch processing (unfortunately separate if
  // statement) is this legato-dependant?
  if (_event->m_trigger_env) {
...
void PolySection::keyUp(PolyKeyEvent *_event) {
  ...
  updateNotePitch(_event->m_voiceId);
  if (_event->m_trigger_env) {
    stopEnvelopes(_event->m_voiceId, m_note_pitch[_event->m_voiceId],
                  _event->m_velocity);
  }
  m_key_active--;
}
...
void PolySection::evalVoiceFade(const float _from, const float _range) {
  // floating approach: the number of faded keys is ceil(_range), the fade slope
  // is depending on floating range (previously, faded keys were floor(_range),
  // the fade slope was depending on floor(_range) potential problem: gain in
  // precision is hardly audible (even with other part muted), so is perceived
  // general improvement
  ...
  const int32_t fadeStart =
      start +
      (m_fadeIncrement *
       (1 + (m_fadeStart + (m_fadeIncrement * static_cast<int32_t>(_from)))));
  const int32_t fadeEnd =
      fadeStart + (m_fadeIncrement * static_cast<int32_t>(std::ceil(_range)));
  ...
  for (int32_t i = fadeStart; i != fadeEnd; i += m_fadeIncrement) {
    ...
    // float fadeValue = 0.5f - (0.5f * NlToolbox::Math::sinP3_wrap(0.5f * (fade
    // - 0.5f)));
  ...
  // phase 3: 0%
  for (int32_t i = fadeEnd; i != m_fadeEnd + m_fadeIncrement;
       i += m_fadeIncrement) {
    if ((i < 0) || (i >= C15::Config::virtual_key_count))
      break; // safety mechanism
...
void PolySection::evalVoiceFadeInterpolated(const float _from,
                                            const float _range) {
  // floating and interpolated approach: the number of faded keys is
  // ceil(_range), ...
  // ... fade slope is now interpolated (determined by _range), using both
  // floor(_range) and ceil(_range) as separate slope ramps for interpolation,
  // ...
  // ... generating slightly larger fade ramp values when _range has fractional
  // part (previously, approach was floating only) potential improvement: gain
  // in precision should be more audible, so should perceived general
  // improvement

  const int32_t start = m_fadeStart,
                fadeStart =
                    start + (m_fadeIncrement *
                             (1 + start +
                              (m_fadeIncrement * static_cast<int32_t>(_from)))),
  ...
  // as described, we now evaluate two separate ramps with different slopes and
  // interpolate by crossfading according to the fractional
  const float fractional = _range - static_cast<float>(lowerRange),
              slope[2] = {(1.0f / (static_cast<float>(lowerRange) + 1.0f)),
                          (1.0f / (static_cast<float>(upperRange) + 1.0f))};
  ...
  for (int32_t i = fadeStart; i != fadeEnd; i += m_fadeIncrement) {
    ...
    // sine implementation (keep for further testing):
    //    float fadeValue[2] = { (0.5f - (0.5f *
    //    NlToolbox::Math::sinP3_wrap(0.5f * (fade[0] - 0.5f)))),
    //                           (0.5f - (0.5f *
    //                           NlToolbox::Math::sinP3_wrap(0.5f * (fade[1] -
    //                           0.5f)))) };
    ...
    m_key_levels[i] = ((1.0f - fractional) * fadeValue[0]) +
                      (fractional * fadeValue[1]); // now interpolated
  }
...
float PolySection::evalScale(const uint32_t _voiceId) {
  ...
  const int32_t pos_start =
      pos - static_cast<int32_t>(m_globalsignals->get(
                C15::Signals::Global_Signals::Scale_Base_Key_Start));
  const int32_t pos_dest =
      pos - static_cast<int32_t>(m_globalsignals->get(
                C15::Signals::Global_Signals::Scale_Base_Key_Dest));
  // form scale offset index by signal offset + cyclic index -- twice
  const uint32_t index_start =
      static_cast<uint32_t>(C15::Signals::Global_Signals::Scale_Offset_0) +
      (static_cast<uint32_t>(pos_start) % 12);
  const uint32_t index_dest =
      static_cast<uint32_t>(C15::Signals::Global_Signals::Scale_Offset_0) +
      (static_cast<uint32_t>(pos_dest) % 12);
  // retrieve base key smoother fade position and crossfade offsets
  const float fade =
      m_globalsignals->get(C15::Signals::Global_Signals::Scale_Base_Key_Pos);
  return ((1.0f - fade) * m_globalsignals->get(index_start)) +
         (fade * m_globalsignals->get(index_dest));
}
...
void PolySection::postProcess_poly_audio(const uint32_t _voiceId,
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_A_PM_Self_Env_A, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_A_Tmb, _voiceId),
          tmp_env) *
          tmp_amt);
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_A_PM_B_Env_B, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_B_Tmb, _voiceId),
          tmp_env) *
          tmp_amt);
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_A_PM_FB_Env_C, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Clip, _voiceId),
          tmp_env) *
          tmp_amt);
  // poly shp a signals
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Shp_A_Drive_Env_A, _voiceId,
      (NlToolbox::Crossfades::unipolarCrossFade(
           1.0f,
           m_signals.get(C15::Signals::Truepoly_Signals::Env_A_Tmb, _voiceId),
           tmp_env) *
  ...
  tmp_amt =
      tmp_env < 0.0f
          ...
          : m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Clip, _voiceId);
  m_signals.set(
      C15::Signals::Truepoly_Signals::Shp_A_FB_Env_C, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          m_signals.get(C15::Signals::Truepoly_Signals::Env_G_Sig, _voiceId),
          tmp_amt, std::abs(tmp_env)));
  // poly osc b signals
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_B_PM_Self_Env_B, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_B_Tmb, _voiceId),
          tmp_env) *
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_B_PM_A_Env_A, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_A_Tmb, _voiceId),
          tmp_env) *
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_B_PM_FB_Env_C, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          1.0f,
          m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Clip, _voiceId),
          tmp_env) *
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Shp_B_Drive_Env_B, _voiceId,
      (NlToolbox::Crossfades::unipolarCrossFade(
           1.0f,
           m_signals.get(C15::Signals::Truepoly_Signals::Env_B_Tmb, _voiceId),
           tmp_env) *
  ...
  tmp_amt =
      tmp_env < 0.0f
          ...
          : m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Clip, _voiceId);
  m_signals.set(
      C15::Signals::Truepoly_Signals::Shp_B_FB_Env_C, _voiceId,
      NlToolbox::Crossfades::unipolarCrossFade(
          m_signals.get(C15::Signals::Truepoly_Signals::Env_G_Sig, _voiceId),
          tmp_amt, std::abs(tmp_env)));
  // comb filter
  ...
  m_signals.set(C15::Signals::Truepoly_Signals::Comb_Flt_Freq_Env_C, _voiceId,
                tmp_env);
  tmp_env =
      m_signals.get(C15::Signals::Truepoly_Signals::Env_G_Sig,
                    _voiceId); // application of gate signal (with 10ms release)
  tmp_amt = NlToolbox::Crossfades::unipolarCrossFade(
      m_comb_decay_times[0][_voiceId], m_comb_decay_times[1][_voiceId],
      tmp_env);
  m_signals.set(C15::Signals::Truepoly_Signals::Comb_Flt_Decay, _voiceId,
                tmp_amt); // decay now fully formed as min/max
  // unison (phase)
  const float phase =
      m_smoothers.get(C15::Smoothers::Poly_Audio::Unison_Phase) *
...
#if POTENTIAL_IMPROVEMENT_FAST_PITCH_PROCESSING ==                             \
    __POTENTIAL_IMPROVEMENT_ENABLED__
  postProcess_poly_pitch(
      _voiceId,
      m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Uncl, _voiceId));
#endif
  // temporary variables
  const uint32_t uIndex = m_unison_index[_voiceId];
  const float basePitch = m_base_pitch[_voiceId],
              notePitch = m_note_pitch[_voiceId];
  float tmp_lvl, tmp_pan;
  // feedback mixer
  tmp_lvl = m_smoothers.get(C15::Smoothers::Poly_Fast::FB_Mix_Lvl);
  tmp_pan =
      std::min(m_convert->eval_level(
                   m_smoothers.get(C15::Smoothers::Poly_Slow::FB_Mix_Lvl_KT) *
                   notePitch),
               env_clip_peak);
  m_signals.set(C15::Signals::Truepoly_Signals::FB_Mix_Lvl, _voiceId,
                tmp_lvl * tmp_pan);
  // output mixer
  const float poly_pan =
      m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_Key_Pan) *
          (basePitch - 6.0f) +
      (m_smoothers.get(C15::Smoothers::Poly_Fast::Unison_Pan) *
       m_spread.m_pan[m_uVoice][uIndex]);
  tmp_lvl = m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_A_Lvl);
  tmp_pan = std::clamp(
      m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_A_Pan) + poly_pan,
      0.0f, 1.0f);
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_A_Left, _voiceId,
                tmp_lvl * (1.0f - tmp_pan));
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_A_Right, _voiceId,
                tmp_lvl * tmp_pan);
  tmp_lvl = m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_B_Lvl);
  tmp_pan = std::clamp(
      m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_B_Pan) + poly_pan,
      0.0f, 1.0f);
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_B_Left, _voiceId,
                tmp_lvl * (1.0f - tmp_pan));
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_B_Right, _voiceId,
                tmp_lvl * tmp_pan);
  tmp_lvl = m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_Comb_Lvl);
  tmp_pan = std::clamp(
      m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_Comb_Pan) + poly_pan,
      0.0f, 1.0f);
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_Comb_Left, _voiceId,
                tmp_lvl * (1.0f - tmp_pan));
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_Comb_Right, _voiceId,
                tmp_lvl * tmp_pan);
  tmp_lvl = m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_SVF_Lvl);
  tmp_pan = std::clamp(
      m_smoothers.get(C15::Smoothers::Poly_Fast::Out_Mix_SVF_Pan) + poly_pan,
      0.0f, 1.0f);
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_SVF_Left, _voiceId,
                tmp_lvl * (1.0f - tmp_pan));
  m_signals.set(C15::Signals::Truepoly_Signals::Out_Mix_SVF_Right, _voiceId,
                tmp_lvl * tmp_pan);
}
...
void PolySection::postProcess_mono_fast() {
  ...
  m_signals.set(C15::Signals::Quasipoly_Signals::SV_Flt_LBH_1,
                m_svf_LBH1Curve.applyCurve(tmp_lvl));
  m_signals.set(C15::Signals::Quasipoly_Signals::SV_Flt_LBH_2,
                m_svf_LBH2Curve.applyCurve(tmp_lvl));
  ...
  m_signals.set(C15::Signals::Quasipoly_Signals::SV_Flt_Par_2,
                (0.7f * tmp_lvl) + tmp_inv);
...
void PolySection::postProcess_poly_slow(const uint32_t _voiceId) {
#if POTENTIAL_IMPROVEMENT_FAST_PITCH_PROCESSING ==                             \
    __POTENTIAL_IMPROVEMENT_DISABLED__
  postProcess_poly_pitch(
      _voiceId,
      m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Uncl, _voiceId));
#endif
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_A_Fluct_Env_C, _voiceId,
  ...
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_B_Fluct_Env_C, _voiceId,
  ...
  unitSign = m_smoothers.get(C15::Smoothers::Poly_Slow::Comb_Flt_Decay) < 0
                 ? -0.001f
                 : 0.001f;
  envMod = 1.0f - m_comb_decayCurve.applyCurve(m_smoothers.get(
                      C15::Smoothers::Poly_Slow::Comb_Flt_Decay_Gate));
  unitMod =
      std::abs(m_smoothers.get(C15::Smoothers::Poly_Slow::Comb_Flt_Decay));
  unitPitch = (-0.5f * notePitch * keyTracking);
  unitValue = m_signals.get(C15::Signals::Truepoly_Signals::Comb_Flt_Freq)
                  [_voiceId]; // decay now fully formed as min/max
  m_comb_decay_times[0][_voiceId] = m_combfilter.calcDecayGain(
      m_convert->eval_level(unitPitch + (unitMod * envMod)) * unitSign,
      unitValue);
  m_comb_decay_times[1][_voiceId] = m_combfilter.calcDecayGain(
      m_convert->eval_level(unitPitch + unitMod) * unitSign, unitValue);
  ...
  m_signals.set(C15::Signals::Truepoly_Signals::Comb_Flt_AP_Freq, _voiceId,
                evalNyquist(440.0f * unitPitch *
                            m_convert->eval_lin_pitch(
                                69.0f + (notePitch * keyTracking) + envMod)));
  ...
  m_signals.set(C15::Signals::Truepoly_Signals::Comb_Flt_LP_Freq, _voiceId,
                evalNyquist(440.0f * unitPitch *
                            m_convert->eval_lin_pitch(
                                69.0f + (notePitch * keyTracking) + envMod)));
  // state variable filter
  keyTracking = m_smoothers.get(C15::Smoothers::Poly_Slow::SV_Flt_Res_KT) *
                m_svf_resFactor;
  ...
  unitPitch = m_svf_resCurve.applyCurve(
      std::clamp(m_smoothers.get(C15::Smoothers::Poly_Slow::SV_Flt_Res) +
                     envMod + (notePitch * keyTracking),
                 0.0f, 1.0f));
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_Res_Damp, _voiceId,
                2.0f - (2.0f * unitPitch));
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_Res_FMax, _voiceId,
                0.7352f + (0.2930f * unitPitch * (1.3075f + unitPitch)));
  // feedback mixer
  m_signals.set(
      C15::Signals::Truepoly_Signals::FB_Mix_HPF, _voiceId,
...
void PolySection::postProcess_mono_slow() {
  ...
  // osc a
  m_signals.set(
      C15::Signals::Quasipoly_Signals::Osc_A_Chirp,
      evalNyquist(m_smoothers.get(C15::Smoothers::Poly_Slow::Osc_A_Chirp) *
                  440.0f));
  // osc b
  m_signals.set(
      C15::Signals::Quasipoly_Signals::Osc_B_Chirp,
      evalNyquist(m_smoothers.get(C15::Smoothers::Poly_Slow::Osc_B_Chirp) *
                  440.0f));
}
...
void PolySection::postProcess_poly_pitch(const uint32_t _voiceId,
  ...
  envMod =
      _envC * m_smoothers.get(C15::Smoothers::Poly_Slow::Osc_A_Pitch_Env_C);
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_A_Freq, _voiceId,
      evalNyquist((*m_reference) *
                  m_convert->eval_osc_pitch(
                      unitPitch + (notePitch * keyTracking) + envMod)));
  // osc b (pitch only)
  ...
  envMod =
      _envC * m_smoothers.get(C15::Smoothers::Poly_Slow::Osc_B_Pitch_Env_C);
  m_signals.set(
      C15::Signals::Truepoly_Signals::Osc_B_Freq, _voiceId,
      evalNyquist((*m_reference) *
                  m_convert->eval_osc_pitch(
                      unitPitch + (notePitch * keyTracking) + envMod)));
  // comb filter (pitch_as_delay_time and comb_bypass only) - maybe, take ap
  // filter freq into account as well?
  ...
  m_signals.set(C15::Signals::Truepoly_Signals::Comb_Flt_Freq, _voiceId,
                evalNyquist((*m_reference) * unitPitch *
                            m_convert->eval_lin_pitch(
                                69.0f + (notePitch * keyTracking))));
  ...
  unitPitch =
      (*m_reference) * m_smoothers.get(C15::Smoothers::Poly_Slow::SV_Flt_Cut);
  ...
  unitValue = evalNyquist(
      unitPitch * m_convert->eval_lin_pitch(69.0f + (notePitch * keyTracking) +
                                            envMod + unitSpread));
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_F1_Cut, _voiceId,
                unitValue);
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_F1_FM, _voiceId,
                unitValue * unitMod);
  unitValue = evalNyquist(
      unitPitch * m_convert->eval_lin_pitch(69.0f + (notePitch * keyTracking) +
                                            envMod - unitSpread));
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_F2_Cut, _voiceId,
                unitValue);
  m_signals.set(C15::Signals::Truepoly_Signals::SV_Flt_F2_FM, _voiceId,
                unitValue * unitMod);
...
void PolySection::postProcess_poly_key(const uint32_t _voiceId) {
  postProcess_poly_pitch(
      _voiceId,
      m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Uncl, _voiceId));
  // - key override event now fully separated from clock, env c peak level is
  // active immediately when attack == 0 LATER: consider re-formulating
  // envelopes (keydown already affects signal) and rendering them after
  // everything else (avoiding this override) pitch, unison, temporary variables
  const uint32_t uIndex = m_unison_index[_voiceId];
  const float basePitch = m_base_pitch[_voiceId],
              notePitch = m_note_pitch[_voiceId];
  float keyTracking, unitPitch, envMod, unitSign, unitMod, unitValue, tmp_lvl,
      tmp_pan;
  // osc a
  envMod = m_smoothers.get(C15::Smoothers::Poly_Slow::Osc_A_Fluct_Env_C);
  m_signals.set(
...
void PolySection::startEnvelopes(const uint32_t _voiceId, const float _pitch,
                                 const float _vel) {
  float time, timeKT, dest, levelVel, attackVel, decay1Vel, decay2Vel, levelKT,
      peak, unclipped;
  // env a
  ...
  decay1Vel =
      m_smoothers.get(C15::Smoothers::Poly_Sync::Env_A_Dec_1_Vel) * _vel;
  decay2Vel =
      m_smoothers.get(C15::Smoothers::Poly_Sync::Env_A_Dec_2_Vel) * _vel;
  levelKT = m_smoothers.get(C15::Smoothers::Poly_Sync::Env_A_Lvl_KT) * _pitch;
  if (levelVel < 0.0f) {
    peak = std::min(m_convert->eval_level((_vel * levelVel) + levelKT),
                    env_clip_peak);
  } else {
    peak =
  ...
  decay1Vel =
      m_smoothers.get(C15::Smoothers::Poly_Sync::Env_B_Dec_1_Vel) * _vel;
  decay2Vel =
      m_smoothers.get(C15::Smoothers::Poly_Sync::Env_B_Dec_2_Vel) * _vel;
...
void PolySection::stopEnvelopes(const uint32_t _voiceId, const float _pitch,
                                const float _vel) {
...
void PolySection::updateEnvLevels() {
  // env a
  m_env_a.setSegmentDest(ElevatingEnv::SegmentId::Decay1,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_BP));
  m_env_a.setSegmentDest(ElevatingEnv::SegmentId::Decay2,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_Sus));
  // env b
  m_env_b.setSegmentDest(ElevatingEnv::SegmentId::Decay1,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_BP));
  m_env_b.setSegmentDest(ElevatingEnv::SegmentId::Decay2,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_Sus));
  // env c
  m_env_c.setSegmentDest(LoopableEnv::SegmentId::Decay1,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_C_BP));
  m_env_c.setSegmentDest(LoopableEnv::SegmentId::Decay2,
                         m_smoothers.get(C15::Smoothers::Poly_Fast::Env_C_Sus));
}

void PolySection::updateEnvTimes() {
  PolyValue time, inf;
  // env a
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Att) *
         m_env_a.mTimeFactor[ElevatingEnv::SegmentId::Attack];
  m_env_a.setSegmentDx(ElevatingEnv::SegmentId::Attack, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Dec_1) *
         m_env_a.mTimeFactor[ElevatingEnv::SegmentId::Decay1];
  m_env_a.setSegmentDx(ElevatingEnv::SegmentId::Decay1, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Dec_2) *
  ...
  // env b
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Att) *
         m_env_b.mTimeFactor[ElevatingEnv::SegmentId::Attack];
  m_env_b.setSegmentDx(ElevatingEnv::SegmentId::Attack, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Dec_1) *
         m_env_b.mTimeFactor[ElevatingEnv::SegmentId::Decay1];
  m_env_b.setSegmentDx(ElevatingEnv::SegmentId::Decay1, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Dec_2) *
  ...
  // env c
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Att) *
         m_env_c.mTimeFactor[LoopableEnv::SegmentId::Attack];
  m_env_c.setSegmentDx(LoopableEnv::SegmentId::Attack, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Dec_1) *
         m_env_c.mTimeFactor[LoopableEnv::SegmentId::Decay1];
  m_env_c.setSegmentDx(LoopableEnv::SegmentId::Decay1, 1.0f / (time + 1.0f));
  time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Dec_2) *
         m_env_c.mTimeFactor[LoopableEnv::SegmentId::Decay2];
...
void PolySection::updateNotePitch(const uint32_t _voiceId) {
  ...
  m_base_pitch[_voiceId] = ((1.0f - glide) * m_last_key_tune[_voiceId]) +
                           (glide * m_key_tune[_voiceId]) + evalScale(_voiceId);
  m_note_pitch[_voiceId] =
...
void PolySection::testVoiceFadeTable() {
  ...
  for (int32_t i = 0; i < C15::Config::virtual_key_count; i += 16) {
    nltools::Log::info("[", i, "..", i + 15, "]:", m_key_levels[i], "..",
                       m_key_levels[i + 7], "..", m_key_levels[i + 15]);
  }
}
