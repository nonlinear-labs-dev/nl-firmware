...
namespace Engine
{
  ...
  void PolySection::postProcess_mono_audio()
  {
    ...
    const ScalarValue gainA = m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_Gain),
                      gainB = m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_Gain);
    ...
    m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Clip) = m_env_c.getSignal(LoopableEnv::ChannelId::Magnitude);
    m_signals.get(C15::Signals::Truepoly_Signals::Env_C_Uncl)
        = m_env_c.mClipFactor * m_env_c.getSignal(LoopableEnv::ChannelId::Magnitude);
  ...
  void PolySection::postProcess_poly_pitch(const uint32_t _voiceId, const float _envC)
  {
    ...
    m_signals.set(
        C15::Signals::Truepoly_Signals::Comb_Flt_Freq, _voiceId,
  ...
  void PolySection::startEnvelopes(const uint32_t _voiceId, const float _pitch, const float _vel)
  {
    ...
    m_env_a.setCurvature(_voiceId, m_smoothers.get(C15::Smoothers::Poly_Sync::Env_A_Att_Curve));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Dec_1)
        * m_env_a.mTimeFactor[ElevatingEnv::SegmentId::Decay1][_voiceId];
    m_env_a.setSegmentDx(ElevatingEnv::SegmentId::Decay1, _voiceId, 1.0f / (time + 1.0f));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Dec_2)
        * m_env_a.mTimeFactor[ElevatingEnv::SegmentId::Decay2][_voiceId];
    m_env_a.setSegmentDx(ElevatingEnv::SegmentId::Decay2, _voiceId, 1.0f / (time + 1.0f));
    ...
    m_env_b.setCurvature(_voiceId, m_smoothers.get(C15::Smoothers::Poly_Sync::Env_B_Att_Curve));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Dec_1)
        * m_env_b.mTimeFactor[ElevatingEnv::SegmentId::Decay1][_voiceId];
    m_env_b.setSegmentDx(ElevatingEnv::SegmentId::Decay1, _voiceId, 1.0f / (time + 1.0f));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Dec_2)
        * m_env_b.mTimeFactor[ElevatingEnv::SegmentId::Decay2][_voiceId];
    m_env_b.setSegmentDx(ElevatingEnv::SegmentId::Decay2, _voiceId, 1.0f / (time + 1.0f));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Att)
        * m_env_c.mTimeFactor[LoopableEnv::SegmentId::Attack][_voiceId];
    m_env_c.setSegmentDx(LoopableEnv::SegmentId::Attack, _voiceId, 1.0f / (time + 1.0f));
    m_env_c.setSegmentDest(LoopableEnv::SegmentId::Attack, _voiceId,
                           1.0f);  // peaklevel will be applied
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Dec_1)
        * m_env_c.mTimeFactor[LoopableEnv::SegmentId::Decay1][_voiceId];
    m_env_c.setSegmentDx(LoopableEnv::SegmentId::Decay1, _voiceId, 1.0f / (time + 1.0f));
    ...
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Dec_2)
        * m_env_c.mTimeFactor[LoopableEnv::SegmentId::Decay1][_voiceId];
    m_env_c.setSegmentDx(LoopableEnv::SegmentId::Decay2, _voiceId, 1.0f / (time + 1.0f));
    ...
    m_env_c.start(_voiceId, [this](const ScalarValue &_loopFactor) { return m_convert->eval_loopFactor(_loopFactor); });
  ...
  void PolySection::stopEnvelopes(const uint32_t _voiceId, const float _pitch, const float _vel)
  {
    ...
    if(time <= env_highest_finite_time)
    {
      ...
      m_env_a.setSegmentDx(ElevatingEnv::SegmentId::Release, _voiceId, 1.0f / (time + 1.0f));
    }
    else
    {
    ...
    if(time <= env_highest_finite_time)
    {
      ...
      m_env_b.setSegmentDx(ElevatingEnv::SegmentId::Release, _voiceId, 1.0f / (time + 1.0f));
    }
    else
    {
    ...
    if(time <= env_highest_finite_time)
    {
      ...
      m_env_c.setSegmentDx(LoopableEnv::SegmentId::Release, _voiceId, 1.0f / (time + 1.0f));
    }
    else
    {
    ...
    m_env_c.stop(_voiceId, [this](const ScalarValue &_loopFactor) { return m_convert->eval_loopFactor(_loopFactor); });
  ...
  {
    // env a
    m_env_a.setSegmentDest(ElevatingEnv::SegmentId::Decay1, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_BP));
    m_env_a.setSegmentDest(ElevatingEnv::SegmentId::Decay2, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_Sus));
    // env b
    m_env_b.setSegmentDest(ElevatingEnv::SegmentId::Decay1, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_BP));
    m_env_b.setSegmentDest(ElevatingEnv::SegmentId::Decay2, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_Sus));
    // env c
    m_env_c.setSegmentDest(LoopableEnv::SegmentId::Decay1, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_C_BP));
    m_env_c.setSegmentDest(LoopableEnv::SegmentId::Decay2, m_smoothers.get(C15::Smoothers::Poly_Fast::Env_C_Sus));
  }
  ...
  void PolySection::updateEnvTimes()
  {
    PolyValue time, inf;
    // env a
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_A_Att) * m_env_a.mTimeFactor[ElevatingEnv::SegmentId::Attack];
    ...
    m_env_a.setElevation(m_smoothers.get(C15::Smoothers::Poly_Fast::Env_A_Elevate));
    // env b
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_B_Att) * m_env_b.mTimeFactor[ElevatingEnv::SegmentId::Attack];
    ...
    m_env_b.setElevation(m_smoothers.get(C15::Smoothers::Poly_Fast::Env_B_Elevate));
    // env c
    time = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Att) * m_env_c.mTimeFactor[LoopableEnv::SegmentId::Attack];
    m_env_c.setSegmentDx(LoopableEnv::SegmentId::Attack, 1.0f / (time + 1.0f));
    time
        = m_smoothers.get(C15::Smoothers::Poly_Slow::Env_C_Dec_1) * m_env_c.mTimeFactor[LoopableEnv::SegmentId::Decay1];
