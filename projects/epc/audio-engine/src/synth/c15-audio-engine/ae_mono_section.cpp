...
void MonoSection::init(exponentiator *_convert, LayerSignalCollection *_z_self,
                       const float _ms, const float _samplerate,
  ...
  m_flanger_rate_to_decay *=
      NlToolbox::Constants::twopi * m_reciprocal_samplerate;
...
void MonoSection::add_copy_sync_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
  m_smoothers.m_copy_sync.add_copy_id(_smootherId, _signalId);
}

void MonoSection::add_copy_audio_id(const uint32_t _smootherId,
                                    const uint32_t _signalId) {
  m_smoothers.m_copy_audio.add_copy_id(_smootherId, _signalId);
}

void MonoSection::add_copy_fast_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
  m_smoothers.m_copy_fast.add_copy_id(_smootherId, _signalId);
}

void MonoSection::add_copy_slow_id(const uint32_t _smootherId,
                                   const uint32_t _signalId) {
...
void MonoSection::start_audio(const uint32_t _id, const float _dx,
                              const float _dest) {
  m_smoothers.start_audio(_id, _dx, _dest);
}

void MonoSection::start_fast(const uint32_t _id, const float _dx,
                             const float _dest) {
  m_smoothers.start_fast(_id, _dx, _dest);
}

void MonoSection::start_slow(const uint32_t _id, const float _dx,
                             const float _dest) {
  m_smoothers.start_slow(_id, _dx, _dest);
}

void MonoSection::render_audio(const float _left, const float _right,
                               const float _vol, const float _to_other) {
...
void MonoSection::postProcess_audio() {
  ...
  for (uint32_t i = 0; i < traversal->m_length; i++) {
    m_signals.set(traversal->m_signalId[i],
                  m_smoothers.get_audio(traversal->m_smootherId[i]));
  }
  // flanger
  m_flanger_env.tick(0);
  m_flanger_lfo.tick();
  const float tmp_env = (m_flanger_env.getSignal(
                            FlangerEnv::ChannelId::Magnitude, 0)) -
                        1.f,
              tmp_wet =
  ...
  m_signals.set(C15::Signals::Mono_Signals::Flanger_Tremolo_R,
                m_flanger_lfo.m_right);
  m_signals.set(C15::Signals::Mono_Signals::Flanger_LFO_L,
                (tmp_dry * m_flanger_lfo.m_left) + (tmp_wet * tmp_env));
  m_signals.set(C15::Signals::Mono_Signals::Flanger_LFO_R,
                (tmp_dry * m_flanger_lfo.m_right) + (tmp_wet * tmp_env));
}
...
void MonoSection::postProcess_fast() {
  ...
  // temporary variables
  float tmp_val, tmp_dry, tmp_wet, tmp_hi_par, tmp_lo_par, tmp_hi_ser,
      tmp_lo_ser, tmp_fb;
  // flanger
  m_flanger_lfo.m_phaseOffset =
      m_flanger_norm_phase *
      m_smoothers.get(C15::Smoothers::Mono_Slow::Flanger_Phase);
  tmp_fb = m_flanger_fb_curve.applyCurve(
      m_smoothers.get(C15::Smoothers::Mono_Fast::Flanger_Feedback));
  tmp_val = m_smoothers.get(C15::Smoothers::Mono_Fast::Flanger_Cross_FB);
  m_signals.set(C15::Signals::Mono_Signals::Flanger_FB_Local,
                tmp_fb * (1.0f - std::abs(tmp_val)));
  ...
  m_signals.set(C15::Signals::Mono_Signals::Flanger_DRY,
                1.0f - std::abs(tmp_val));
  ...
  m_signals.set(
      C15::Signals::Mono_Signals::Flanger_APF_L,
      evalNyquist(
          m_convert->eval_lin_pitch(
              (m_signals.get(C15::Signals::Mono_Signals::Flanger_LFO_L) *
               tmp_val) +
              tmp_dry) *
          440.0f));
  m_signals.set(
      C15::Signals::Mono_Signals::Flanger_APF_R,
      evalNyquist(
          m_convert->eval_lin_pitch(
              (m_signals.get(C15::Signals::Mono_Signals::Flanger_LFO_R) *
               tmp_val) +
              tmp_dry) *
          440.0f));
  // cabinet
  ...
  tmp_val =
      std::max(m_cabinet_tilt_floor, m_convert->eval_level(0.5f * tmp_fb));
  ...
  tmp_dry =
      NlToolbox::Math::sin(NlToolbox::Constants::halfpi * (1.0f - tmp_val));
  tmp_val = m_smoothers.get(C15::Smoothers::Mono_Fast::Gap_Flt_Bal);
  tmp_hi_par = 0.5f + (0.5f * tmp_val);
  tmp_lo_par = 1.f - tmp_hi_par;
  tmp_hi_par = NlToolbox::Math::sin(NlToolbox::Constants::halfpi * tmp_hi_par) *
               NlToolbox::Constants::sqrt_two;
  tmp_lo_par = NlToolbox::Math::sin(NlToolbox::Constants::halfpi * tmp_lo_par) *
               NlToolbox::Constants::sqrt_two;
  tmp_val *= tmp_val;
  tmp_hi_ser = m_smoothers.get(C15::Smoothers::Mono_Fast::Gap_Flt_Bal) > 0.0f
                   ? tmp_val
                   : 0.0f;
  tmp_lo_ser = m_smoothers.get(C15::Smoothers::Mono_Fast::Gap_Flt_Bal) > 0.0f
                   ? 0.0f
                   : tmp_val;
  if (m_smoothers.get(C15::Smoothers::Mono_Fast::Gap_Flt_Mix) > 0.0f) {
    ...
    m_signals.set(C15::Signals::Mono_Signals::Gap_Flt_HP_Out,
                  tmp_wet * tmp_hi_par);
    m_signals.set(C15::Signals::Mono_Signals::Gap_Flt_LP_Out,
                  tmp_wet * tmp_lo_par);
    ...
    m_signals.set(C15::Signals::Mono_Signals::Gap_Flt_HP_Out,
                  m_signals.get(C15::Signals::Mono_Signals::Gap_Flt_HP_LP) *
                      tmp_val);
    m_signals.set(C15::Signals::Mono_Signals::Gap_Flt_LP_Out,
                  tmp_wet - tmp_val);
    m_signals.set(C15::Signals::Mono_Signals::Gap_Flt_In_Out,
                  (tmp_val * tmp_lo_ser) + tmp_dry);
  }
  // echo
  ...
  m_signals.set(C15::Signals::Mono_Signals::Echo_FB_Local,
                tmp_fb * (1.0f - tmp_val));
  ...
  m_signals.set(C15::Signals::Mono_Signals::Echo_Wet,
                (2.0f * tmp_val) - (tmp_val * tmp_val));
  tmp_val = 1.0f - tmp_val;
  m_signals.set(C15::Signals::Mono_Signals::Echo_Dry,
                (2.0f * tmp_val) - (tmp_val * tmp_val));
  // reverb
...
void MonoSection::postProcess_slow() {
  ...
  // temporary variables
  float tmp_Gap, tmp_Center, tmp_Stereo, tmp_Rate, tmp_val, tmp_fb, tmp_dry,
      tmp_wet;
  // flanger
  ...
  tmp_Center = m_smoothers.get(C15::Smoothers::Mono_Slow::Flanger_Time) *
               1.e-3f * m_samplerate;
  tmp_Stereo =
      m_smoothers.get(C15::Smoothers::Mono_Slow::Flanger_Stereo) * 0.01f;
  m_signals.set(C15::Signals::Mono_Signals::Flanger_Time_L,
                tmp_Center * (1.0f + tmp_Stereo));
  m_signals.set(C15::Signals::Mono_Signals::Flanger_Time_R,
                tmp_Center * (1.0f - tmp_Stereo));
  m_signals.set(
      C15::Signals::Mono_Signals::Flanger_LPF,
      evalNyquist(m_smoothers.get(C15::Smoothers::Mono_Slow::Flanger_Hi_Cut) *
                  440.0f));
  // cabinet
  m_signals.set(
      C15::Signals::Mono_Signals::Cabinet_LPF,
      evalNyquist(m_smoothers.get(C15::Smoothers::Mono_Slow::Cabinet_Hi_Cut) *
                  440.0f));
  m_signals.set(
      C15::Signals::Mono_Signals::Cabinet_HPF,
      evalNyquist(m_smoothers.get(C15::Smoothers::Mono_Slow::Cabinet_Lo_Cut) *
                  440.0f));
  // gap filter
  tmp_Gap =
      (m_smoothers.get(C15::Smoothers::Mono_Fast::Gap_Flt_Mix) < 0.0f ? -1.0f
                                                                      : 1.0f) *
  ...
  m_signals.set(
      C15::Signals::Mono_Signals::Gap_Flt_LF_L,
      evalNyquist(m_convert->eval_lin_pitch(tmp_Center - tmp_Gap - tmp_Stereo) *
                  440.0f));
  m_signals.set(
      C15::Signals::Mono_Signals::Gap_Flt_HF_L,
      evalNyquist(m_convert->eval_lin_pitch(tmp_Center + tmp_Gap - tmp_Stereo) *
                  440.0f));
  m_signals.set(
      C15::Signals::Mono_Signals::Gap_Flt_LF_R,
      evalNyquist(m_convert->eval_lin_pitch(tmp_Center - tmp_Gap + tmp_Stereo) *
                  440.0f));
  m_signals.set(
      C15::Signals::Mono_Signals::Gap_Flt_HF_R,
      evalNyquist(m_convert->eval_lin_pitch(tmp_Center + tmp_Gap + tmp_Stereo) *
                  440.0f));
  // echo
  tmp_Center =
      m_smoothers.get(C15::Smoothers::Mono_Slow::Echo_Time) * m_samplerate;
  tmp_Stereo = m_smoothers.get(C15::Smoothers::Mono_Slow::Echo_Stereo) *
               m_delay_norm_stereo;
  m_signals.set(C15::Signals::Mono_Signals::Echo_Time_L,
                tmp_Center * (1.0f + tmp_Stereo));
  m_signals.set(C15::Signals::Mono_Signals::Echo_Time_R,
                tmp_Center * (1.0f - tmp_Stereo));
  m_signals.set(
      C15::Signals::Mono_Signals::Echo_LPF,
      evalNyquist(m_smoothers.get(C15::Smoothers::Mono_Slow::Echo_Hi_Cut) *
                  440.0f));
  // reverb
  ...
  m_signals.set(C15::Signals::Mono_Signals::Reverb_Feedback,
                4.32f - (3.32f * tmp_fb));
  ...
  m_signals.set(C15::Signals::Mono_Signals::Reverb_Pre,
                m_smoothers.get(C15::Smoothers::Mono_Slow::Reverb_Pre_Dly) *
                    200.0f * m_millisecond);
  ...
  m_signals.set(C15::Signals::Mono_Signals::Reverb_LPF,
                evalNyquist(m_convert->eval_lin_pitch(
                                m_reverb_color_curve_1.applyCurve(tmp_val)) *
                            440.0f));
  m_signals.set(C15::Signals::Mono_Signals::Reverb_HPF,
                evalNyquist(m_convert->eval_lin_pitch(
                                m_reverb_color_curve_2.applyCurve(tmp_val)) *
                            440.0f));
}
