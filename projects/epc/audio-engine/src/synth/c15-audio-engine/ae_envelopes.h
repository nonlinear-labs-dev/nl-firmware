...
namespace Engine
{
  ...
  class Envelopes final
  {
   ...
   private:  // internal trait declarations
    ...
    static inline ScalarValue applyRetrigger(const ScalarValue &_signal, const ScalarValue &_from,
    ...
    static inline ScalarValue applyCurvature(const ScalarValue &_signal, const ScalarValue &_curvature)
    {
    ...
    static inline constexpr CurveType determineCurveType(const bool &_finite, const CurveType &_curve)
    {
      return _finite ? _curve : (CurveType) - (Signed) _curve;
    };
    template <CurveType>
    static inline ScalarValue renderCurve(ScalarValue &_x, ScalarValue &_y, const ScalarValue &_dx,
                                          const ScalarValue &_curvature = 0.0f);
    template <CurveType> static inline bool isTransitionIncomplete(const ScalarValue &_x, const ScalarValue &_y);
    ...
    struct Initializer
    {
      inline constexpr Initializer(const ScalarValue &_dx = {}, const ScalarValue &_dest = {},
    ...
    template <typename Stage, Stage _Start, Stage _Stop = Stage::Idle, Stage _LoopStart = Stage::Idle,
              Stage _LoopBegin = Stage::Idle, Stage _LoopEnd = Stage::Idle>
    struct Segment
    {
      enum Marker : Signed
      {
      ...
      inline constexpr Segment(const CurveType &_curve = CurveType::None, const Marker &_marker = None,
                               const Stage &_next = Stage::Idle, const Initializer &_init = {})
          : mCurve { _curve }
          , mMarker { _marker }
          , mNext { _next }
          , mInit { _init }
    ...
    template <typename, typename, typename, ChannelType, RetriggerType> class Envelope;
   ...
   public:  // exposed usable envelope types
    ...
    class MonoDecay
    {
     ...
     public:
      inline void start(const ScalarValue &_dest)
      {
        mMagnitude = _dest;
      }
      inline void tick()
      {
        mMagnitude *= mDx * (float) (mMagnitude > sRenderMin);
      }
      inline void setDx(const ScalarValue &_dx)
      {
        mDx = 1.0f - _dx;
      }
      inline void reset()
  ...
  // impl Retrigger
  template <>
  inline ScalarValue Envelopes::applyRetrigger<Envelopes::RetriggerType::Soft>(const ScalarValue &_signal,
                                                                               const ScalarValue &, const ScalarValue &)
  {
    // soft: result is current signal
    return _signal;
  }
  template <>
  inline ScalarValue Envelopes::applyRetrigger<Envelopes::RetriggerType::Hard>(const ScalarValue &,
  ...
  template <>
  inline ScalarValue Envelopes::applyRetrigger<Envelopes::RetriggerType::Variable>(const ScalarValue &_signal,
                                                                                   const ScalarValue &_from,
                                                                                   const ScalarValue &_fade)
  {
  ...
  // impl Curve
  template <>
  inline ScalarValue Envelopes::renderCurve<Envelopes::CurveType::Linear>(ScalarValue &_x, ScalarValue &,
                                                                          const ScalarValue &_dx, const ScalarValue &)
  {
  ...
  template <>
  inline ScalarValue Envelopes::renderCurve<Envelopes::CurveType::Exponential>(ScalarValue &, ScalarValue &_y,
  ...
  template <>
  inline ScalarValue Envelopes::renderCurve<Envelopes::CurveType::Polynomial>(ScalarValue &_x, ScalarValue &,
                                                                              const ScalarValue &_dx,
  ...
  template <>
  inline bool Envelopes::isTransitionIncomplete<Envelopes::CurveType::Linear>(const ScalarValue &_x,
                                                                              const ScalarValue &)
  {
    return 1.0f > _x;
  };
  template <>
  inline bool Envelopes::isTransitionIncomplete<Envelopes::CurveType::Exponential>(const ScalarValue &,
                                                                                   const ScalarValue &_y)
  {
    return _y > sRenderMin;
  };
  template <>
  inline bool Envelopes::isTransitionIncomplete<Envelopes::CurveType::Polynomial>(const ScalarValue &_x,
                                                                                  const ScalarValue &)
  {
    return 1.0f > _x;
  };
  ...
  template <typename T, Envelopes::ChannelType C> struct Envelopes::Channel::Data
  {
    ...
    struct Scalar
    {
      ...
      template <typename CB> inline void apply(const VoiceId &_voiceId, const CB &_transformCb)
      {
    ...
    template <typename CB> inline void apply(const CB &_transformCb)
    {
      for(Unsigned c = 0; c < ChannelId::_Length_; c++)
        mData[c] = _transformCb(c);
    }
  ...
  // impl Envelope
  template <typename T, typename SID, typename SegmentData, Envelopes::ChannelType C, Envelopes::RetriggerType R>
  class Envelopes::Envelope
  {
   ...
   protected:
        ...
        , mState {}
    {
      for(Unsigned segmentId = 0; segmentId < sSegments; segmentId++)
      {
        ...
        for(Unsigned voiceId = 0; voiceId < sVoices; voiceId++)
        {
          ...
          transition.mDest.apply(voiceId, [&segment](const Unsigned &) { return segment.mInit.mDest; });
        }
      }
    }
    ...
    // positions
    inline ChannelScalar determineDifference(const VoiceId &_voiceId, const Unsigned &_segmentId) const
    {
      ChannelScalar ret;
      ret.apply(_voiceId, [this, &_voiceId, &_segmentId](const Unsigned &_channelId) {
        return mTransitions[_segmentId].mDest.mData[_channelId][_voiceId] - mState.mStart.mData[_channelId][_voiceId];
      });
      return ret;
    }
    // core rendering algorithm
    inline bool render(const VoiceId &_voiceId, const Unsigned &_segmentId, const ChannelScalar &_difference,
                       const ScalarValue &_mute)
    {
      ...
      switch(curve)
      {
        case CurveType::Linear:
          if((isIncomplete = isTransitionIncomplete<CurveType::Linear>(mState.mX[_voiceId], mState.mY[_voiceId])))
          {
            rendered
                = renderCurve<CurveType::Linear>(mState.mX[_voiceId], mState.mY[_voiceId], transition.mDx[_voiceId]);
          }
          break;
        case CurveType::Exponential:
          if((isIncomplete = isTransitionIncomplete<CurveType::Exponential>(mState.mX[_voiceId], mState.mY[_voiceId])))
          {
            rendered = renderCurve<CurveType::Exponential>(mState.mX[_voiceId], mState.mY[_voiceId],
                                                           transition.mDx[_voiceId]);
        ...
        case CurveType::Polynomial:
          if((isIncomplete = isTransitionIncomplete<CurveType::Polynomial>(mState.mX[_voiceId], mState.mY[_voiceId])))
          {
            rendered = renderCurve<CurveType::Polynomial>(mState.mX[_voiceId], mState.mY[_voiceId],
                                                          transition.mDx[_voiceId], mState.mCurvature[_voiceId]);
          }
          break;
        default:  // idle
          break;
      ...
      mState.mSignal.apply(_voiceId, [this, &_voiceId, &rendered, &_difference](const Unsigned &_channelId) {
        return mState.mStart.mData[_channelId][_voiceId] + (rendered * _difference.mData[_channelId]);
      });
      // determine if transition has finished
      return !isIncomplete && isFinite;
    }
    // set segment destination per channel
    inline void setSegmentDest(const SegmentId &_segmentId, const VoiceId &_voiceId, const ChannelScalar &_dest)
    {
      const ScalarValue &peakLevel = mState.mPeakLevel[_voiceId];
      mTransitions[(Unsigned) _segmentId].mDest.apply(
          _voiceId, [&_dest, &peakLevel](const Unsigned &_channelId) { return peakLevel * _dest[_channelId]; });
    }
    inline void setSegmentDest(const SegmentId &_segmentId, const ChannelValue &_dest)
    {
      mTransitions[(Unsigned) _segmentId].mDest.apply(
          [this, &_dest](const Unsigned &_channelId) { return mState.mPeakLevel * _dest[_channelId]; });
    }
    // progress to next transition/segment
    inline void nextTransition(const VoiceId &_voiceId, const Unsigned &_segmentId)
    {
   ...
   private:
    ...
    // inheritance
    template <RetriggerType Retr> inline void startTransition(const VoiceId &_voiceId, const Unsigned &_segmentId)
    {
      mState.mSegmentId[_voiceId] = (SegmentId) _segmentId;
      mState.mStart.apply(_voiceId, [this, &_voiceId, &_segmentId](const Unsigned &_channelId) {
        return applyRetrigger<Retr>(mState.mSignal.mData[_channelId][_voiceId], mSegments[_segmentId].mInit.mStart,
   ...
   public:
    ...
    inline void start(const VoiceId &_voiceId, const MarkerId &_markerId = MarkerId::Start)
    {
      startTransition<R>(_voiceId, (Unsigned) _markerId);
      tick(_voiceId, 1.0f);
    }
    inline void stop(const VoiceId &_voiceId, const MarkerId &_markerId = MarkerId::Stop)
    {
      startTransition<RetriggerType::Soft>(_voiceId, (Unsigned) _markerId);
    }
    // update segment parameters
    inline void setSegmentDx(const SegmentId &_segmentId, const VoiceId &_voiceId, const ScalarValue &_dx)
    {
    ...
    inline void setSegmentDest(const SegmentId &_segmentId, const VoiceId &_voiceId, const ScalarValue &_dest)
    {
      const ScalarValue &peakLevel = mState.mPeakLevel[_voiceId];
      mTransitions[(Unsigned) _segmentId].mDest.apply(
          _voiceId, [&_dest, &peakLevel](const Unsigned &_channelId) { return peakLevel * _dest; });
    }
    inline void setSegmentDest(const SegmentId &_segmentId, const WrapperData &_dest)
    {
      mTransitions[(Unsigned) _segmentId].mDest.apply(
          [this, &_dest](const Unsigned &_channelId) { return mState.mPeakLevel * _dest; });
    }
    inline void setCurvature(const VoiceId &_voiceId, const ScalarValue &_curvature)
    {
    ...
    inline void setPeakLevel(const VoiceId &_voiceId, const ScalarValue &_peakLevel)
    {
    ...
    inline ScalarValue &getSignal(const ChannelId &_channelId, const VoiceId &_voiceId)
    {
      return mState.mSignal.mData[_channelId][_voiceId];
    }
    inline const ScalarValue &getSignal(const ChannelId &_channelId, const VoiceId &_voiceId) const
    {
      return mState.mSignal.mData[_channelId][_voiceId];
    }
  };
  ...
  template <typename T, Envelopes::RetriggerType R> class Envelopes::ElevatingADBDSR
  {
    ...
    static constexpr bool sElevateableSegments[] = { false, false, true, true, false };
   ...
   public:
    class Impl : public Super, public Super::TimeFactor
    {
     ...
     public:
      ...
      inline void setSegmentDest(const Stage &_segmentId, const VoiceId &_voiceId, const ScalarValue &_dest)
      {
        ...
        if(sElevateableSegments[segmentId])
        {
          Super::setSegmentDest(
              _segmentId, _voiceId,
              { NlToolbox::Crossfades::unipolarCrossFade(_dest, 1.0f, mElevation[ChannelId::Magnitude]),
                NlToolbox::Crossfades::unipolarCrossFade(_dest, 1.0f, mElevation[ChannelId::Timbre]) });
        }
        else
        {
      ...
      inline void setSegmentDest(const Stage &_segmentId, const WrapperData &_dest)
      {
        const Unsigned segmentId = (Unsigned) _segmentId;
        if(sElevateableSegments[segmentId])
        {
  ...
  template <typename T, Envelopes::RetriggerType R> class Envelopes::LoopableADBDSR
  {
    ...
    using SegmentBase = Envelopes::Segment<Stage, Stage::Attack, Stage::Release, Stage::LoopAttack, Stage::LoopDecay1,
                                           Stage::LoopDecay2>;
    using MarkerId = typename SegmentBase::Marker;
    struct Segment : public SegmentBase
    {
      inline constexpr Segment(const CurveType &_curve = CurveType::None, const MarkerId &_marker = MarkerId::None,
                               const Stage &_sibling = Stage::Idle, const Stage &_next = Stage::Idle,
                               const Initializer &_init = {})
          : SegmentBase { _curve, _marker, _next, _init }
          , mSibling { _sibling }
      {
      }
    ...
    static constexpr Segment sSegments[] = {
      {},
      { CurveType::Polynomial, Segment::Start, Sibling::LoopAttack, Stage::Decay1, { 0.0f, 1.0f } },
      { CurveType::Linear, Segment::None, Sibling::LoopDecay1, Stage::Decay2 },
      { CurveType::InfiniteExponential, Segment::None, Sibling::LoopDecay2, Stage::Idle },
      { CurveType::Exponential, Segment::Stop, Sibling::Idle, Stage::Idle },
      { CurveType::Polynomial, Segment::LoopStart, Sibling::Attack, Stage::LoopDecay1, { 0.0f, 1.0f } },
      { CurveType::Linear, Segment::LoopBegin, Sibling::Decay1, Stage::LoopDecay2 },
   ...
   public:
    class Impl : public Super, public Super::ClipFactor, public Super::TimeFactor
    {
      ...
      inline ChannelScalar determineDifference(const VoiceId &_voiceId, const Unsigned &_segmentId,
                                               const ScalarValue &_mute) const
      {
        ChannelScalar ret;
        const ScalarValue mute = mLoopState.mLoop[_voiceId] == Loop::Release ? _mute : 1.0f;
        const ScalarValue decay = mute * mLoopState.mDecayAndRelease[_voiceId];
        ret.apply(_voiceId, [this, &_voiceId, &_segmentId, &decay](const Unsigned &_channelId) {
          const ScalarValue dest = Super::mTransitions[_segmentId].mDest.mData[_channelId][_voiceId];
          const ScalarValue start = Super::mState.mStart.mData[_channelId][_voiceId];
          return (dest * decay) - start;
        });
        return ret;
      }
      inline void applyLoopDecay(ScalarValue &_value, const ScalarValue &_factor)
      {
     ...
     public:
      ...
      inline void tick(const VoiceId &_voiceId, const ScalarValue &_mute = 1.0f)
      {
        ...
        const ChannelScalar difference = determineDifference(_voiceId, segmentId, _mute);
        const MarkerId markerId = Super::mSegments[segmentId].mMarker;
        const bool isComplete = Super::render(_voiceId, segmentId, difference, _mute);
        if(isComplete)
        {
          if(markerId == MarkerId::LoopEnd)
          {
            applyLoopDecay(mLoopState.mDecayAndRelease[_voiceId], mLoopState.mFactor[_voiceId]);
          }
          Super::nextTransition(_voiceId, (Unsigned) Super::mSegments[segmentId].mNext);
        }
      }
      template <typename CB> inline void start(const VoiceId &_voiceId, const CB &_loopScaleCb)
      {
        ...
        else
        {
          ...
          mLoopState.mFactor[_voiceId] = _loopScaleCb(mLoopState.mValue > 100.0f ? 100.0f : mLoopState.mValue);
          Super::start(_voiceId, MarkerId::LoopStart);
        }
      }
      template <typename CB> inline void stop(const VoiceId &_voiceId, const CB &_loopScaleCb)
      {
      ...
      inline void setSegmentDx(const Stage &_stage, const VoiceId &_voiceId, const ScalarValue &_dx)
      {
      ...
      inline void setSegmentDx(const Stage &_stage, const WrapperData &_dx)
      {
      ...
      inline void setSegmentDest(const Stage &_stage, const VoiceId &_voiceId, const ScalarValue &_dest)
      {
      ...
      inline void setSegmentDest(const Stage &_stage, const WrapperData &_dest)
      {
