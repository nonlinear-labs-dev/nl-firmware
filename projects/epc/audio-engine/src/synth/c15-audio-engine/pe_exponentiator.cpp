...
void exponentiator::init() {
  ...
  for (i = 0; i <= dsp_expon_lin_pitch_range; i++) {
    ...
    exp =
        (static_cast<float>(i) + s_linear_pitch_from - s_freqExponent_offset) *
        s_scaleFreqExponent;
  ...
  for (i = 1; i < 20; i++) {
    /* determine exponent of equation */
    exp = (hyperfloor(static_cast<float>(i) + s_oscillator_pitch_from) -
           s_freqExponent_offset) *
          s_scaleFreqExponent;
    /* determine power (base ^ exponent) */
  ...
  for (i = 20; i <= dsp_expon_osc_pitch_range; i++) {
    /* determine exponent of equation */
    exp = (static_cast<float>(i) + s_oscillator_pitch_from -
           s_freqExponent_offset) *
          s_scaleFreqExponent;
    /* determine power (base ^ exponent) */
    m_oscillator_pitch_table[i] = pow(s_freqBase, exp);
