...
GlobalSection::GlobalSection() { m_out_l = m_out_r = 0.0f; }

void GlobalSection::init(const float _sampleInc) { m_sampleInc = _sampleInc; }

void GlobalSection::add_copy_sync_id(const uint32_t _smootherId,
                                     const uint32_t _signalId) {
  m_smoothers.m_copy_sync.add_copy_id(_smootherId, _signalId);
}

void GlobalSection::add_copy_audio_id(const uint32_t _smootherId,
                                      const uint32_t _signalId) {
  m_smoothers.m_copy_audio.add_copy_id(_smootherId, _signalId);
}

void GlobalSection::add_copy_fast_id(const uint32_t _smootherId,
                                     const uint32_t _signalId) {
  m_smoothers.m_copy_fast.add_copy_id(_smootherId, _signalId);
}

void GlobalSection::add_copy_slow_id(const uint32_t _smootherId,
                                     const uint32_t _signalId) {
...
void GlobalSection::start_audio(const uint32_t _id, const float _dx,
                                const float _dest) {
  m_smoothers.start_audio(_id, _dx, _dest);
}

void GlobalSection::start_fast(const uint32_t _id, const float _dx,
                               const float _dest) {
  m_smoothers.start_fast(_id, _dx, _dest);
}

void GlobalSection::start_slow(const uint32_t _id, const float _dx,
                               const float _dest) {
...
void GlobalSection::start_base_key(const float _dx, const float _dest) {
  ...
  const float start =
      m_signals.get(C15::Signals::Global_Signals::Scale_Base_Key_Start) +
      (m_baseKeyDiff * baseKeyValue);
...
void GlobalSection::render_audio(const float _leftI, const float _rightI,
                                 const float _leftII, const float _rightII) {
  // common dsp: smoothers, signal post processing
  m_smoothers.render_audio();
  postProcess_audio();
  // #3003: FX/Part Pan
  auto panned =
      StereoPanning::fromDualStereoSignal(_leftI, _rightI, _leftII, _rightII);
  panned *= StereoPanning::fromPanning(
      m_smoothers.get(C15::Smoothers::Global_Fast::Master_Pan));
...
void GlobalSection::resetDSP() { m_out_l = m_out_r = 0.0f; }
...
void GlobalSection::postProcess_fast() {
  ...
  // #2996: Serial FX
  const float serialFX =
      m_smoothers.get(C15::Smoothers::Global_Fast::Master_Serial_FX);
  m_signals.set(C15::Signals::Global_Signals::Master_FX_I_to_II,
                std::max(0.f, serialFX));
  m_signals.set(C15::Signals::Global_Signals::Master_FX_II_to_I,
                std::max(0.f, -serialFX));
}
