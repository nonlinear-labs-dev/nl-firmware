...
Preset::Preset(UpdateDocumentContributor *parent, const EditBuffer &editBuffer, bool copyUUID)
    : Preset(parent, editBuffer, copyUUID ? editBuffer.getUUIDOfLastLoadedPreset() : Uuid())
{
}
...
void Preset::copyFrom(UNDO::Transaction *transaction, const AttributesOwner *other)
{
  ...
  if(auto preset = dynamic_cast<const Preset*>(other)) {
    transaction->addUndoSwap(m_name, preset->getName());
    transaction->addUndoSwap(m_voiceGroupLabels[0], preset->getVoiceGroupName(VoiceGroup::I));
    transaction->addUndoSwap(m_voiceGroupLabels[1], preset->getVoiceGroupName(VoiceGroup::II));
    transaction->addUndoSwap(m_type, preset->getType());
  }
