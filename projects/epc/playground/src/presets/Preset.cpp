...
void Preset::load(UNDO::Transaction *transaction, const Glib::RefPtr<Gio::File> &presetPath)
{
  ...
  updatePresetHashtags(transaction);
}
...
void Preset::setAutoGeneratedAttributes(UNDO::Transaction *transaction)
{
  ...
  if(Application::exists())
  {
    ...
    super::setAttribute(transaction, "DeviceName", settings->getSetting<DeviceName>()->get());
    super::setAttribute(transaction, "StoreTime", TimeTools::getAdjustedIso(adj));
...
{
  auto monoEnabledI = findParameterByID({ C15::PID::Mono_Grp_Enable, VoiceGroup::I }, false);
  auto monoEnabledII = findParameterByID({ C15::PID::Mono_Grp_Enable, VoiceGroup::II }, false);
...
{
  auto unisonVoicesI = findParameterByID({ C15::PID::Unison_Voices, VoiceGroup::I }, false);
  auto unisonVoicesII = findParameterByID({ C15::PID::Unison_Voices, VoiceGroup::II }, false);
...
namespace
{
  bool isScaleNotDefault(const Preset *p)
  {
    for(const auto &s :
        { C15::PID::Scale_Base_Key, C15::PID::Scale_Offset_1, C15::PID::Scale_Offset_2, C15::PID::Scale_Offset_3,
    ...
    {
      if(auto param = p->findParameterByID({ s, VoiceGroup::Global }, false))
      {
        if(param->getValue() != 0)
          return true;
      }
...
Glib::ustring Preset::getHashtags() const
{
  return m_hashtags;
}

void Preset::updatePresetHashtags(UNDO::Transaction *transaction)
{
  transaction->addUndoSwap(transaction, m_hashtags, createHashtags());
}

Glib::ustring Preset::createHashtags() const
{
  const auto monoEnabled = isMonoActive();
  const auto unisonEnabled = isUnisonActive();
  const auto isScaleEnabled = isScaleNotDefault(this);

  std::stringstream ss;
  ss << "#" << toString(getType()) << ", ";

  if(monoEnabled)
    ss << "#Mono" << (unisonEnabled || isScaleEnabled ? ", " : " ");
  else
    ss << "#Poly" << (unisonEnabled || isScaleEnabled ? ", " : " ");

  if(unisonEnabled)
    ss << "#Unison" << (isScaleEnabled ? ", " : " ");
