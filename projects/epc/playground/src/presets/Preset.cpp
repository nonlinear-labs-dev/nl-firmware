...
void Preset::setAutoGeneratedAttributes(UNDO::Transaction *transaction)
{
  ...
  if(Application::exists())
  {
    ...
    super::setAttribute(transaction, "Hashtags", getHashtags());
...
bool Preset::matchesQuery(const SearchQuery &query) const
{
  return query.iterate([&](const auto &part, const auto &fields) {
    ...
    static_assert(static_cast<int>(SearchQuery::Fields::Hashtags) == 3, "");

    std::array<Glib::ustring, 4> entries = { getName().lowercase(), getAttribute("Comment", "").lowercase(),
                                             getAttribute("DeviceName", "").lowercase(), getHashtags().lowercase() };
...
Glib::ustring Preset::getHashtags() const
{
  ...
  parts.emplace_back("#" + toString(getType()));

  //mono unison scale
  if(PresetPresenter::isMonoActive(this))
  {
    parts.emplace_back("#Mono");
  }
  else
  {
    parts.emplace_back("#Poly");
  }

  if(PresetPresenter::isUnisonActive(this))
  {
    parts.emplace_back("#Unison");
  }
  else
  {
