    ...
    , m_presetManager(presetManager)
{
  ...
  addAction("append-preset-to-bank",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
  ...
  addAction("create-new-bank-from-preset",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
  ...
  addAction("create-new-bank-from-presets",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
              auto csv = request->get("presets");
              auto x = request->get("x");
  ...
  addAction("drop-presets-on-bank",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
              Glib::ustring bankUUID = request->get("bank");
              Glib::ustring csv = request->get("presets");
  ...
  addAction("overwrite-preset-with-bank",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
  ...
  addAction("set-preset-attribute",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
              auto presetUUID = request->get("uuid");
  ...
  addAction("insert-bank-in-cluster",
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
            ...
            [&](const std::shared_ptr<NetworkRequest>& request) mutable
            {
              ...
              if(auto bank = m_presetManager.findBank(Uuid { uuid }))
              {
                if(auto attached = m_presetManager.findBank(bank->getAttachedToBankUuid()))
                {
                  ...
                  auto scope = presetManager.getUndoScope().startTransaction("Detached Bank '%0' from '%1'",
                                                                             bank->getName(true), parentBankName);
                  auto transaction = scope->getTransaction();
