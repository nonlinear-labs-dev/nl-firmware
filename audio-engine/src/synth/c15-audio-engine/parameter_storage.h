...
struct Macro_Control_Parameter : Proto_Parameter
{
    ...
    float m_base, m_mod = 0.0f;
    bool m_bidir = false;
};

template<typename Scale, typename Clock, typename Signal>
struct Modulateable_Parameter : Proto_Parameter
{
    float m_amount, m_base, m_ceil, m_unclipped = 0.0f;
...
template<typename Scale, typename Clock, typename Signal, class HW, class GP>
struct Global_Parameter_Storage
{
    Hardware_Source_Parameter m_source[static_cast<uint32_t>(HW::_LENGTH_)];
    Unmodulateable_Parameter<Scale, Clock, Signal> m_direct[static_cast<uint32_t>(GP::_LENGTH_)];
};

template<typename Scale, typename Clock, typename Signal, class HA, class MC, class TP, class DP>
struct Layer_Parameter_Storage
{
    ...
    Modulateable_Parameter<Scale, Clock, Signal> m_target[static_cast<uint32_t>(TP::_LENGTH_)];
    Unmodulateable_Parameter<Scale, Clock, Signal> m_direct[static_cast<uint32_t>(DP::_LENGTH_)];
