...
struct decoder
{
    ...
    dual_id_list m_selectedVoices;                  // ID List of selected Voices
    polyDual_id_list m_selectedParams;              // ID List of selected Parameters
    id_list m_listTraversal[lst_number_of_lists];   // ID Lists for Preset Recall and Key Events
    ...
    void unsigned28upper(const uint32_t _data0, const uint32_t _data1);                     // TCD argument parsing: unsigned 28 bit values (upper 14 bit)
    void signed28upper(const uint32_t _data0, const uint32_t _data1);                       // TCD argument parsing: signed 28 bit values (upper 14 bit)
    int32_t apply28lower(const uint32_t _data0, const uint32_t _data1);                     // TCD argument parsing: 28 bit values (lower 14 bit)

    uint32_t selectionEvent(const uint32_t _from, const uint32_t _to, const uint32_t _id);  // TCD Voice and Parameter selection event evaluation

    uint32_t traverseRecall();                                                              // TCD List traversal: Preset Recall Events
    uint32_t traverseKeyEvent();                                                            // TCD List traversal: Key Events
};
