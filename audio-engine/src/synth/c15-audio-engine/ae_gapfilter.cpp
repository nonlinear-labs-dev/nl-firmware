...
void ae_gapfilter::set(float *_signal)
{
    ...
    //************************** Biquad Highpass R ***************************//
    frequency = _signal[GAP_HFR];
    tmpVar_1 = (frequency - m_freqScale_min) * (1.f / (m_freqScale_max - m_freqScale_min));
    tmpVar_1 = std::clamp(tmpVar_1, -1.f, 1.f);

    resonance = _signal[GAP_RES] * tmpVar_1;
...
void ae_gapfilter::apply(float _rawSample_L, float _rawSample_R, float *_signal)
{
    ...
    //****************************** Lowpass L ******************************//
    tmpVar = hp_sample * _signal[GAP_HPLP] + _rawSample_L * _signal[GAP_INLP];
    ...
    //****************************** Lowpass R ******************************//
    tmpVar = hp_sample * _signal[GAP_HPLP] + _rawSample_R * _signal[GAP_INLP];
