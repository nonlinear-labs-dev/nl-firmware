...
void ae_gapfilter::set(ParameterStorage &params)
{
  ...
  //************************** Biquad Highpass R ***************************//
  frequency = params[GAP_HFR];
  tmpVar_1 = (frequency - m_freqScale_min) * (1.f / (m_freqScale_max - m_freqScale_min));
  tmpVar_1 = std::clamp(tmpVar_1, -1.f, 1.f);

  resonance = params[GAP_RES] * tmpVar_1;
...
void ae_gapfilter::apply(float _rawSample_L, float _rawSample_R, ParameterStorage &params)
{
  ...
  //****************************** Lowpass L ******************************//
  tmpVar = hp_sample * params[GAP_HPLP] + _rawSample_L * params[GAP_INLP];
  ...
  lp_sample = tmpVar * params[GAP_LPOUT];
  hp_sample *= params[GAP_HPOUT];
  m_out_L = hp_sample + lp_sample + (_rawSample_L * params[GAP_INOUT]);
  ...
  //****************************** Lowpass R ******************************//
  tmpVar = hp_sample * params[GAP_HPLP] + _rawSample_R * params[GAP_INLP];
  ...
  lp_sample = tmpVar * params[GAP_LPOUT];
  hp_sample *= params[GAP_HPOUT];
  m_out_R = hp_sample + lp_sample + (_rawSample_R * params[GAP_INOUT]);
}
