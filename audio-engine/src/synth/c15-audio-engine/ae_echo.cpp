...
{
  float omega = std::clamp(signals[SignalLabel::DLY_LPF], m_freqClip_min, m_freqClip_max);
...
void ae_echo::apply(float _rawSample_L, float _rawSample_R, SignalStorage &signals)
{
  float tmpVar;

  //***************************** Left Channel *****************************//
  tmpVar = (_rawSample_L * signals[SignalLabel::DLY_SND]) + (m_stateVar_L * signals[SignalLabel::DLY_FB_LOC])
      + (m_stateVar_R * signals[SignalLabel::DLY_FB_CR]);

  m_buffer_L[m_buffer_indx] = tmpVar;

  tmpVar = signals[SignalLabel::DLY_TL] - m_lp2hz_stateVar_L;  // 2Hz LP
  ...
  m_out_L = NlToolbox::Crossfades::crossFade(_rawSample_L, m_out_L, signals[SignalLabel::DLY_DRY],
                                             signals[SignalLabel::DLY_WET]);

  //**************************** Right Channel *****************************//
  tmpVar = (_rawSample_R * signals[SignalLabel::DLY_SND]) + (m_stateVar_R * signals[SignalLabel::DLY_FB_LOC])
      + (m_stateVar_L * signals[SignalLabel::DLY_FB_CR]);

  m_buffer_R[m_buffer_indx] = tmpVar;

  tmpVar = signals[SignalLabel::DLY_TR] - m_lp2hz_stateVar_R;  // 2Hz LP
  ...
  m_out_R = NlToolbox::Crossfades::crossFade(_rawSample_R, m_out_R, signals[SignalLabel::DLY_DRY],
                                             signals[SignalLabel::DLY_WET]);
