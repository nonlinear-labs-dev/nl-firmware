...
void ae_svfilter_proto::apply(float _sampleA, float _sampleB, float _sampleComb, SignalStorage &signals)
{
  ...
  float omega = signals[SignalLabel::SVF_F1_CUT] * m_warpConst_2PI;
  ...
  omega = (signals[SignalLabel::SVF_F1_CUT] + fmVar * signals[SignalLabel::SVF_F1_FM]) * m_warpConst_2PI;
  omega = std::min(omega, test_svf_fm_limit);  /// initially 1.9f
  ...
  omega = signals[SignalLabel::SVF_F2_CUT] * m_warpConst_2PI;
  omega = std::min(omega, test_svf_fm_limit);  /// initially 1.9f
