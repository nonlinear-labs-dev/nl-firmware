...
void ae_soundgenerator::set(SignalStorage &params)
{
  ...
  //*************************** Chirp Filter B *****************************//
  m_chiB_omega = params[SignalLabel::OSC_B_CHI] * m_warpConst_PI;
...
void ae_soundgenerator::generate(float _feedbackSample, SignalStorage &params)
{
  ...
  //**************************** Modulation A ******************************//
  float oscSampleA = m_oscA_selfmix * params[SignalLabel::OSC_A_PMSEA];
  oscSampleA = oscSampleA + m_oscB_crossmix * params[SignalLabel::OSC_A_PMBEB];
  oscSampleA = oscSampleA + m_feedback_phase * params[SignalLabel::OSC_A_PMFEC];
  ...
  oscSampleA += params[SignalLabel::OSC_A_PHS] + params[SignalLabel::UN_PHS];  // NEW Phase Offset
  ...
  float osc_freq = params[SignalLabel::OSC_A_FRQ];
  m_oscA_phaseInc
      = ((m_OscA_randVal_float * params[SignalLabel::OSC_A_FLUEC] * osc_freq) + osc_freq) * m_sample_interval;
  ...
  //**************************** Modulation B ******************************//
  float oscSampleB = m_oscB_selfmix * params[SignalLabel::OSC_B_PMSEB];
  oscSampleB = oscSampleB + m_oscA_crossmix * params[SignalLabel::OSC_B_PMAEA];
  oscSampleB = oscSampleB + m_feedback_phase * params[SignalLabel::OSC_B_PMFEC];
  ...
  oscSampleB += params[SignalLabel::OSC_B_PHS] + params[SignalLabel::UN_PHS];  // NEW Phase Offset
  ...
  osc_freq = params[SignalLabel::OSC_B_FRQ];
  m_oscB_phaseInc
      = ((m_OscB_randVal_float * params[SignalLabel::OSC_B_FLUEC] * osc_freq) + osc_freq) * m_sample_interval;
  ...
  //******************************* Shaper A *******************************//
  float shaperSampleA = oscSampleA * params[SignalLabel::SHP_A_DRVEA];
  tmpVar = shaperSampleA;

  shaperSampleA = NlToolbox::Math::sinP3_wrap(shaperSampleA);
  shaperSampleA = NlToolbox::Others::threeRanges(shaperSampleA, tmpVar, params[SignalLabel::SHP_A_FLD]);

  tmpVar = shaperSampleA * shaperSampleA + (-0.5f);

  shaperSampleA = NlToolbox::Others::parAsym(shaperSampleA, tmpVar, params[SignalLabel::SHP_A_ASM]);

  //******************************* Shaper B *******************************//
  float shaperSampleB = oscSampleB * params[SignalLabel::SHP_B_DRVEB];
  tmpVar = shaperSampleB;

  shaperSampleB = NlToolbox::Math::sinP3_wrap(shaperSampleB);
  shaperSampleB = NlToolbox::Others::threeRanges(shaperSampleB, tmpVar, params[SignalLabel::SHP_B_FLD]);

  tmpVar = shaperSampleB * shaperSampleB + (-0.5f);

  shaperSampleB = NlToolbox::Others::parAsym(shaperSampleB, tmpVar, params[SignalLabel::SHP_B_ASM]);

  //****************************** Crossfades ******************************//
  m_oscA_selfmix = NlToolbox::Crossfades::bipolarCrossFade(oscSampleA, shaperSampleA, params[SignalLabel::OSC_A_PMSSH]);
  m_oscA_crossmix
      = NlToolbox::Crossfades::bipolarCrossFade(oscSampleA, shaperSampleA, params[SignalLabel::OSC_B_PMASH]);

  m_oscB_selfmix = NlToolbox::Crossfades::bipolarCrossFade(oscSampleB, shaperSampleB, params[SignalLabel::OSC_B_PMSSH]);
  m_oscB_crossmix
      = NlToolbox::Crossfades::bipolarCrossFade(oscSampleB, shaperSampleB, params[SignalLabel::OSC_A_PMBSH]);

  m_out_A = NlToolbox::Crossfades::bipolarCrossFade(oscSampleA, shaperSampleA, params[SignalLabel::SHP_A_MIX]);
  m_out_B = NlToolbox::Crossfades::bipolarCrossFade(oscSampleB, shaperSampleB, params[SignalLabel::SHP_B_MIX]);

  //******************* Envelope Influence (Magnitudes) ********************//
  m_out_A *= params[SignalLabel::ENV_A_MAG];
  m_out_B *= params[SignalLabel::ENV_B_MAG];

  //**************************** Feedback Mix ******************************//
  tmpVar = _feedbackSample * params[SignalLabel::SHP_A_FBEC];
  m_out_A = NlToolbox::Crossfades::unipolarCrossFade(m_out_A, tmpVar, params[SignalLabel::SHP_A_FBM]);

  tmpVar = _feedbackSample * params[SignalLabel::SHP_B_FBEC];
  m_out_B = NlToolbox::Crossfades::unipolarCrossFade(m_out_B, tmpVar, params[SignalLabel::SHP_B_FBM]);

  //************************** Ring Modulation *****************************//
  tmpVar = m_out_A * m_out_B;

  m_out_A = NlToolbox::Crossfades::unipolarCrossFade(m_out_A, tmpVar, params[SignalLabel::SHP_A_RM]);
  m_out_B = NlToolbox::Crossfades::unipolarCrossFade(m_out_B, tmpVar, params[SignalLabel::SHP_B_RM]);
}
