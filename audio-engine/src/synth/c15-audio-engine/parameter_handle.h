...
struct ClockHandle
{
    ...
    inline void init(const uint32_t _samplerate)
    {
        ...
        m_div_slow = _samplerate / C15::Config::clock_rates[3];
    }
    inline void render()
    {
...
struct TimeHandle
{
    ...
    inline void init(const uint32_t _samplerate)
    {
        ...
        m_convert[1] = 1.0f;
        m_convert[2] = rate / static_cast<float>(C15::Config::clock_rates[2]);
        m_convert[3] = rate / static_cast<float>(C15::Config::clock_rates[3]);
    }
...
template<typename Scale, typename Section, typename Clock, typename Signal, class Layer, class HW, class GP, class HA, class MC, class TP, class DP>
struct ParameterHandle
{
    ...
    inline void init_modulateable(const C15::ParameterDescriptor _element)
    {
        for(uint32_t i = 0; i < m_layer_count; i++)
        {
            ...
            param->m_polarity = _element.m_polarity;
            param->m_section = _element.m_smoother.m_section;
    ...
    inline Unmodulateable_Parameter<Scale, Section, Clock, Signal>* get_direct(const uint32_t _layerId, const uint32_t _id)
    {
    ...
    Layer_Parameter_Storage<Scale, Section, Clock, Signal, HA, MC, TP, DP> m_layer[static_cast<uint32_t>(Layer::_LENGTH_)];
    Global_Parameter_Storage<Scale, Section, Clock, Signal, HW, GP> m_global;
    const uint32_t
        m_layer_count = static_cast<uint32_t>(Layer::_LENGTH_),
