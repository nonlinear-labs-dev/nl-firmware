...
namespace Detail {
  ...
  public:
    RecallEditBufferSerializer2(EditBuffer* eb, VoiceGroup vg) : Serializer(toString(vg)), m_editBuffer{eb}, m_voiceGroup{vg} {
    }
...
void RecallEditBufferSerializer::readTagContent(Reader &reader) const
{
  reader.onTag(toString(VoiceGroup::I), [&](const Attributes& attr) mutable {
    return new Detail::RecallEditBufferSerializer2(m_editBuffer, VoiceGroup::I);
  });

  reader.onTag(toString(VoiceGroup::II), [&](const Attributes& attr) mutable {
    return new Detail::RecallEditBufferSerializer2(m_editBuffer, VoiceGroup::II);
  });
}
