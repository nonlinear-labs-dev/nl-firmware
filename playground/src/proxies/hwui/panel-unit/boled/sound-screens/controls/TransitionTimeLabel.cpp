...
TransitionTimeLabel::TransitionTimeLabel (const Rect &rect) :
    super (rect)
{
  Application::get ().getSettings ()->getSetting<TransitionTime> ()->onChange (sigc::mem_fun (this, &TransitionTimeLabel::onSettingChanged));
  Application::get ().getHWUI ()->onModifiersChanged (sigc::hide(sigc::mem_fun (this, &TransitionTimeLabel::onModifiersChanged)));
}
