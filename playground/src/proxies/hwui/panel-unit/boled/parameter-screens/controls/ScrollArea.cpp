  ...
  m_content (content)
{
  ...
  addControl (new Scrollbar (content, Rect (position.getWidth() - c_scrollbarWidth, 0, c_scrollbarWidth, position.getHeight())));
...
void ScrollArea::scroll (int diff)
{
  ...
  m_content->setPosition(Rect(2, pos, getPosition().getWidth() - c_scrollbarWidth, m_content->getPosition().getHeight()));
}
