...
DisplayEditor::DisplayEditor() : ControlWithChildren(Rect(0,0,0,0)), SetupEditor(), m_entrys{} {
}

std::pair<LeftAlignedLabel*, LeftAlignedLabel*> DisplayEditor::addEntry(const Glib::ustring& key,
                                                                        const Glib::ustring& name,
                                                                        const Glib::ustring& value) {
  m_order.push_back(key);
  m_entrys[key] = std::make_pair(new LeftAlignedLabel(name, Rect(0,0,50,12)), new LeftAlignedLabel(value, Rect(0,0,70,12)));
