    ...
    super (pos)
{
  ...
  addControl (
      new UndoTransactionNameControl (Rect (border + siblingsWidth, border, pos.getWidth () - 2 * border - 2 * siblingsWidth, height)));
  addControl (new NextUndoTransactionSiblingControl (Rect (pos.getWidth () - border - siblingsWidth, border, siblingsWidth, height)));
}
