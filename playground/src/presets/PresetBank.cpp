...
void PresetBank::undoableStorePreset(UNDO::Scope::tTransactionPtr transaction, size_t pos, tPresetPtr preset)
{
  tPresetPtr presetCopy(createPreset());
  presetCopy->copyFrom(transaction, preset.get(), true);
  presetCopy->setAutoGeneratedAttributes();

  auto swapData = UNDO::createSwapData(presetCopy);

  transaction->addSimpleCommand([ = ] (UNDO::Command::State) mutable
  {
    swapData->swapWith (m_presets[pos]);
    onChange();
  });

  if(preset == getParent()->getEditBuffer())
    getParent()->getEditBuffer()->undoableSetLoadedPresetInfo(transaction, m_presets[pos]);
}
...
void PresetBank::undoableStorePreset(size_t pos, shared_ptr<EditBuffer> editBuffer)
{
  ...
  undoableStorePreset(transaction, pos, editBuffer);
}

void PresetBank::undoableStorePreset(UNDO::Scope::tTransactionPtr transaction, size_t pos, shared_ptr<EditBuffer> editBuffer)
{
  tPresetPtr editBufferAsPreset(createPreset());
  editBufferAsPreset->setName("New Preset");
  editBufferAsPreset->copyFrom(transaction, editBuffer.get(), true);
  editBufferAsPreset->setAutoGeneratedAttributes();

  auto swapData = UNDO::createSwapData(editBufferAsPreset);
  ...
  editBuffer->undoableSetLoadedPresetInfo(transaction, m_presets[pos]);
  editBuffer->resetModifiedIndicator(transaction);
}
...
void PresetBank::assignDefaultPosition()
{
  ...
  if(!banks.empty())
  {
    ...
    m_X = to_string(stod(rightMost->getX()) + 300);
    m_Y = rightMost->getY();
  }
