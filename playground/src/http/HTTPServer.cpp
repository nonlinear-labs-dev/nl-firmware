...
void HTTPServer::initializeServer ()
{
  ...
  soup_server_add_websocket_handler (m_server, "/ws", nullptr, nullptr, (SoupServerWebsocketCallback) &HTTPServer::webSocket, this,
      nullptr);
  soup_server_run_async (m_server);
}

void HTTPServer::webSocket (SoupServer *server, SoupWebsocketConnection *connection, const char *pathStr, SoupClientContext *client,
                            HTTPServer *pThis)
{
...
void HTTPServer::serverCallback (SoupServer *server, SoupMessage *msg, const char *path, GHashTable *query, SoupClientContext *context,
                                 HTTPServer *pThis)
{
  try
  {
