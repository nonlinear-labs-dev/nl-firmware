...
tDisplayValue CombDecayBipolarMsScaleConverter::controlPositionToDisplay (
  const tControlPositionValue &cpValue) const
{
...
Glib::ustring CombDecayBipolarMsScaleConverter::controlPositionToDisplayJS () const
{
  ...
  s   << "return $wnd.formatDimension((cpValue < 0 ? -1 : 1) * (cpValue == 0 ? 0 : Math.exp(" << log_min << " + Math.abs(cpValue) * " << logDiff << ")), \""
      <<  dimensionStringizer << "\", withUnit);";
