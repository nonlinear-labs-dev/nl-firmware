...
tDisplayValue BipolarParabolic100PercentScaleConverter::controlPositionToDisplay (const tControlPositionValue &cpValue) const
{
...
{
  stringstream s;
  s << "return $wnd.formatDimension(cpValue * Math.abs(cpValue) * 100, \"" << getDimension ().getStingizerJS () << "\", withUnit);";
  return s.str ();
}
