...
PhysicalControlParameter::PhysicalControlParameter (ParameterGroup *group, uint16_t id, const ScaleConverter *scaling, tDisplayValue def,
...
void PhysicalControlParameter::setCPFromHwui (UNDO::Scope::tTransactionPtr transaction, const tControlPositionValue &cpValue)
{
...
void PhysicalControlParameter::setCPFromWebUI (UNDO::Scope::tTransactionPtr transaction, const tControlPositionValue &cpValue)
{
  Parameter::setCPFromWebUI (transaction, cpValue);
}

void PhysicalControlParameter::onValueChanged (Initiator initiator, tControlPositionValue oldValue, tControlPositionValue newValue)
{
...
void PhysicalControlParameter::loadFromPreset (UNDO::Scope::tTransactionPtr transaction, const tControlPositionValue &value)
{
}
...
void PhysicalControlParameter::setUiSelectedModulationRouter (int pos)
{
  ...
  if (current != pos)
  {
    ...
    if (auto u = dynamic_pointer_cast<PanelUnitParameterEditMode> (Application::get ().getHWUI ()->getPanelUnit ().getUsageMode ()))
    {
...
void PhysicalControlParameter::undoableRandomize (UNDO::Scope::tTransactionPtr transaction, Initiator initiator, double amount)
{
}
