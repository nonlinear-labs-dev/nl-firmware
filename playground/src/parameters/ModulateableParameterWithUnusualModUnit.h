...
class ModulateableParameterWithUnusualModUnit : public ModulateableParameter
{
  ...
  public:
    ModulateableParameterWithUnusualModUnit (ParameterGroup *group, uint16_t id,
                                             const ScaleConverter *scaling, const ScaleConverter *modAmountScaling,
                                             tDisplayValue def, int coarseDenominator,
                                             int fineDenominator);

    virtual void undoableIncrementMCAmount (UNDO::Scope::tTransactionPtr transaction, int inc, ButtonModifiers modifiers) override;
