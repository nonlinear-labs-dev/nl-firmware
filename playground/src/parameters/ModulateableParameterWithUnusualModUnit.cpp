...
ModulateableParameterWithUnusualModUnit::ModulateableParameterWithUnusualModUnit (ParameterGroup *group, uint16_t id,
...
void ModulateableParameterWithUnusualModUnit::undoableIncrementMCAmount (UNDO::Scope::tTransactionPtr transaction, int inc, ButtonModifiers modifiers)
{
  double denominator = modifiers[ButtonModifier::FINE] ? getModulationAmountFineDenominator() : getModulationAmountCoarseDenominator ();
...
void ModulateableParameterWithUnusualModUnit::writeDocProperties (Writer &writer, UpdateDocumentContributor::tUpdateID knownRevision) const
{
...
{
  return m_modAmountScaling->getDimension().stringize (m_modAmountScaling->controlPositionToDisplay (getModulationAmount()));
}
